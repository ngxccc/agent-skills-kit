...
/**
 * Resolve protocol/rules file path with canonical process path preference and backward compat
...
function resolveRulesPath(filename, configDirName = '.claude') {
  ...
  const legacyFilenames = Object.entries(PROTOCOL_FILENAME_MAP)
    ...
    .map(([legacyFilename]) => legacyFilename);
  ...
  if (fs.existsSync(localProtocolPath))
    return `process/development-protocols/${canonicalFilename}`;
  for (const fallbackFilename of fallbackFilenames) {
    ...
    if (fs.existsSync(localRulesPath))
      return `${configDirName}/rules/${fallbackFilename}`;
    if (fs.existsSync(globalRulesPath))
      return `~/.claude/rules/${fallbackFilename}`;
  }
  ...
  for (const fallbackFilename of fallbackFilenames) {
    ...
    if (fs.existsSync(localWorkflowsPath))
      return `${configDirName}/workflows/${fallbackFilename}`;
    if (fs.existsSync(globalWorkflowsPath))
      return `~/.claude/workflows/${fallbackFilename}`;
  }
...
function buildInjectionScopeKey({ baseDir } = {}) {
  const cwdKey =
    normalizePath(path.resolve(baseDir || process.cwd())) || process.cwd();
  return cwdKey;
}
...
function hasRecentInjection(scopeState, now = Date.now()) {
  ...
  return (
    Number.isFinite(injectedTs) && now - injectedTs < RECENT_INJECTION_TTL_MS
  );
}
...
function hasPendingInjection(scopeState, now = Date.now()) {
  ...
  return (
    Number.isFinite(pendingTs) && now - pendingTs < PENDING_INJECTION_TTL_MS
  );
}
...
function wasTranscriptRecentlyInjected(transcriptPath, scopeKey = null) {
  try {
    ...
    return tail.some(
      (line) =>
        line === `- CWD: ${scopeKey}` ||
        line === `- Working directory: ${scopeKey}`,
    );
  } catch {
    return false;
  }
...
function buildContextSection(sessionId) {
  ...
  try {
    ...
    if (data.percent >= CRITICAL_THRESHOLD) {
      lines.push(
        `- **CRITICAL:** Context nearly full. Before compaction hits:`,
      );
      lines.push(
        `  1. Update TodoWrite with current progress (completed + remaining)`,
      );
...
function buildReminderContext({
  ...
  const cfg =
    config || loadConfig({ includeProject: false, includeAssertions: false });
  ...
  const params = {
    ...
    plansPath: effectiveBaseDir
      ...
      : plansPathRel,
    docsPath: effectiveBaseDir
      ? path.join(effectiveBaseDir, docsPathRel)
      : docsPathRel,
