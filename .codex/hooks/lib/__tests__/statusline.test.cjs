...
test("processEntry hydrates native task id from TaskCreate tool_result", () => {
  ...
  assertEquals(
    latestTodos[0].id,
    "task-123",
    "TaskCreate result should hydrate native task id",
  );
  assertEquals(
    latestTodos[0].status,
    "in_progress",
...
test("getGitInfo timeout does not cache null result", () => {
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "git-timeout-no-cache-"),
  );
  ...
  const hash = crypto
    ...
    .digest("hex")
    .slice(0, 8);
  const cachePath = path.join(os.tmpdir(), `ag-git-cache-${hash}.json`);
  ...
  try {
    ...
    assertEquals(
      result,
      null,
      "Timeout with no stale cache should return null",
    );
    assertFalse(
      fs.existsSync(cachePath),
      "Timeout should not write null cache",
    );
  } finally {
...
test("getGitInfo timeout reuses stale cache when available", () => {
  ...
  try {
    ...
    assertEquals(
      result.branch,
      "stale",
      "Timeout should fall back to stale cache data",
    );
