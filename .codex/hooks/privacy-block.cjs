...
(async () => {
  try {
    ...
    /**
     * Format block message with approval instructions and JSON marker for the assistant approval flow
    ...
    function formatBlockMessage(filePath) {
      ...
      // JSON marker for the assistant to parse and surface in its approval UX
      const promptData = {
        type: 'PRIVACY_PROMPT',
        file: filePath,
        basename: basename,
...
\x1b[90m@@PRIVACY_PROMPT_END@@\x1b[0m
  ...
  \x1b[34mAssistant:\x1b[0m Surface the approval prompt above to the user, then:
  \x1b[32mIf "Yes":\x1b[0m Retry the exact tool call with an approval-prefixed path such as "APPROVED:${filePath}"
