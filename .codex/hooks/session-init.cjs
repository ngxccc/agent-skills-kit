  ...
  function cleanupOrphanedShadowedSkills() {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          process.stderr.write(
            `[session-init] Failed to process "${entry.name}": ${error.message}\n`,
          );
        }
      }
      ...
      if (
        ...
        fs.readdirSync(shadowedDir).length === 0
      ) {
      ...
      process.stderr.write(
        `[session-init] Shadowed cleanup error: ${error.message}\n`,
      );
      return { restored, skipped, kept };
    }
  }
  ...
  async function main() {
    ...
    try {
      ...
      if (
        sessionStateEnabled &&
        sessionId &&
        shouldWarmStatuslineCache(source, existingSession?.statusline)
      ) {
      ...
      const namePattern = resolveNamingPattern(
        config.plan,
        staticEnv.gitBranch,
      );
      ...
      console.log(
        `Session ${source}. ${buildContextOutput(config, detections, resolved, staticEnv.gitRoot)}`,
      );
      ...
      if (hasCleanup) {
        ...
        console.log(
          `Recovered orphaned .shadowed/ directory from disabled skill-dedup hook.`,
        );
      ...
      if (teamInfo) {
        console.log(
          `[i] Agent Team detected: "${teamInfo.teamName}" (${teamInfo.memberCount} members)`,
        );
        console.log(
          `    Team config: ~/.claude/teams/${teamInfo.teamName}/config.json`,
        );
        console.log(
          `    Use the team orchestration skill for parallel coordination templates.`,
        );
      }
      ...
      if (staticEnv.gitRoot && staticEnv.gitRoot !== process.cwd()) {
        console.log(
          `📁 Subdirectory mode: Plans/docs will be created in current directory`,
        );
