...
const highLevelPathTests = [
  ...
  {
    path: '/home/user/worktrees/myproject',
    expected: true,
    desc: 'worktree root',
  },
...
for (const test of broadPatternTests) {
  ...
  if (success) {
    console.log(
      `\x1b[32m✓\x1b[0m ${test.desc}: "${test.pattern}" -> ${result ? 'BROAD' : 'OK'}`,
    );
    passed++;
  } else {
    console.log(
      `\x1b[31m✗\x1b[0m ${test.desc}: expected ${test.expected ? 'BROAD' : 'OK'}, got ${result ? 'BROAD' : 'OK'}`,
    );
    failed++;
  }
}
...
for (const test of highLevelPathTests) {
  ...
  if (success) {
    console.log(
      `\x1b[32m✓\x1b[0m ${test.desc}: "${test.path}" -> ${result ? 'HIGH_LEVEL' : 'SPECIFIC'}`,
    );
    passed++;
  } else {
    console.log(
      `\x1b[31m✗\x1b[0m ${test.desc}: expected ${test.expected ? 'HIGH_LEVEL' : 'SPECIFIC'}, got ${result ? 'HIGH_LEVEL' : 'SPECIFIC'}`,
    );
    failed++;
  }
}
...
for (const test of integrationTests) {
  ...
  if (success) {
    console.log(
      `\x1b[32m✓\x1b[0m ${test.desc} -> ${result.blocked ? 'BLOCKED' : 'ALLOWED'}`,
    );
    passed++;
  } else {
    console.log(
...
if (suggestions.length > 0 && suggestions.some((s) => s.includes('src/'))) {
  console.log(
    `\x1b[32m✓\x1b[0m suggestions for **/*.ts include src-scoped patterns`,
  );
  passed++;
} else {
  console.log(
    `\x1b[31m✗\x1b[0m suggestions should include src-scoped patterns`,
  );
  failed++;
}
