...
_This meta harness turns any AI coding agent into a spec-driven engineering team that researches, plans, ships production-grade code, and self-improves its memory to survive context-rotting even 6 months later._
...
<p>
  ...
  <br>
  <p><em>Not decorative. When you run <code>ag-setup</code>, parallel agents scan your codebase,<br>
...
> **Run this inside your project folder.** Open a terminal and `cd` into the project you want the harness installed into _before_ running the command — it installs into the current directory.
>
...
└── process/                 # 🧠 Created by ag-setup (not install)
    └── ...
```

- **Fresh project?** Installs the full harness, then `ag-setup` studies your codebase
- **Existing `.claude/` config?** Backs up to `.agent-skills-backup/`, installs fresh, restores your `settings.json`
- **Existing `process/` directory?** Never touched by install — `ag-setup` handles migration intelligently
- **Existing `CLAUDE.md`?** Backed up as `CLAUDE.md.pre-agent-skills`, harness version installed
...
The agent doesn't just read your code — it **studies other repositories** to learn how they solved similar problems (`ag-xia`).
...
Before routing any request, scans <strong>32 skills</strong> and matches keywords. Say "add webhook support" and <code>ag-security</code> + <code>ag-scenario</code> surface automatically.
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
| --------------------- | ----------------------------------------------- | -------------------------------------- |
| 🔍 **RESEARCH**       | Read-only fact gathering — codebase + web       | _(automatic on feature requests)_      |
...
| 🧠 **UPDATE PROCESS** | Capture learnings, update context, archive plan | _(recommended after non-trivial work)_ |
...
→ Skill discovery surfaces: ag-scenario, ag-security
...
→ Routes to ag-debugger → evidence gathering → competing hypotheses
...
You: "improve test coverage to 80% using ag-autoresearch"
...
<code>ag-predict</code> — Architect, Security, Performance, UX, and Devil's Advocate debate your plan. Produces a <strong>GO / CAUTION / STOP</strong> verdict before you write a line of code.
</td>
<td width="50%" valign="top">
<h1>🎲</h1>
<strong>12-Dimension Edge Case Generator</strong><br><br>
<code>ag-scenario</code> — Decomposes any feature across 12 dimensions (user types, input extremes, timing, scale, state, env, errors, auth, data, integrations, compliance, business logic). Outputs usable as test specs.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-security</code> — Dual-methodology security audit with dependency auditing, secret detection, and <strong>auto-fix mode</strong> that sorts by severity and fixes Critical first with regression guards.
</td>
</tr>
</table>
...
<code>ag-autoresearch</code> — Set a goal, walk away. Iterative git-backed loop: make ONE atomic change → commit → measure → keep or revert. Stuck detection after 5 consecutive discards triggers strategy shifts.
</td>
<td width="50%" valign="top">
<h1>👥</h1>
<strong>Parallel Agent Teams</strong><br><br>
<code>ag-team</code> — Multiple agents working <strong>simultaneously</strong> with git worktree isolation. Research in parallel, execute in parallel, review in parallel, debug adversarially.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-debugger</code> — Gathers evidence first → forms 2-3 competing hypotheses → systematically tests each → documents elimination path. <strong>Never guesses — proves.</strong>
</td>
</tr>
</table>
...
> 💡 Six months from now, when someone asks _"why did we build auth this way?"_, the answer is in `completed/`. Not lost in a Slack thread.
...
| 🧪 **Auditable**      | `ag-audit-context` verifies routing and consistency               |
...
| 💡 `ag-innovate-agent`       | Brainstorm 2-3 approaches. Must produce decision summary before PLAN            |
| 📋 `ag-plan-agent`           | Write spec with anti-rationalization guards. "I already know how" is not a plan |
| ⚡ `ag-execute-agent`        | Implement per plan. 50% check-in, deviation protocol, self-review               |
| ⏩ `ag-fast-mode-agent`      | Compressed RESEARCH→INNOVATE→PLAN with mandatory safety pause                   |
| 🧠 `ag-update-process-agent` | 7-phase mandatory checklist including stale artifact scanning                   |
...
| 🧪 `ag-tester`          | Diff-aware. Only runs affected tests. Auto-escalates on config changes |
| 🔎 `ag-code-reviewer`   | Edge case scout BEFORE review. N+1 detection, auth path validation     |
| ✨ `ag-code-simplifier` | Clarity refactoring without behavior change                            |
| 🎨 `ag-ui-ux-designer`  | Design-aware frontend. Can spawn research subagent mid-execution       |
| 📦 `ag-git-manager`     | Logical commit splitting from `touched_files`. Refuses unknown files   |
...
**🧠 Planning** — `ag-predict` (5-persona debate) · `ag-scenario` (12-dimension edge cases)
...
**🎨 Frontend** — `ag-frontend-design` · `ag-web-testing` · `ag-nextjs` · `ag-nextjs-bundle-optimizer` · `ag-nextjs-ppr-patterns` · `ag-react-hook-form` · `ag-react-hook-form-audit` · `ag-tailwind` · `ag-tailwind-refactor` · `ag-tailwind-responsive-ui` · `ag-tailwind-ui-refactor` · `ag-ui-design`
...
> 💡 Some skills (like `ag-xia`) were inspired by [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G) by [@mrgoonie](https://github.com/mrgoonie). We focused on fewer, deeper skills instead of 80+.
...
- 🐛 [Report a bug](https://github.com/ngxccc/agent-skills-kit/issues/new?template=1.bug_report.yml)
- 💡 [Request a feature](https://github.com/ngxccc/agent-skills-kit/issues/new?template=2.feature_request.yml)
- ⚡ [Submit a skill](https://github.com/ngxccc/agent-skills-kit/issues/new?template=3.skill_submission.yml)
- 🌐 [Add a translation](https://github.com/ngxccc/agent-skills-kit/issues/new?template=5.translation.yml)
...
This project is cloned and customized from [vibecode-pro-max-kit](https://github.com/withkynam/vibecode-pro-max-kit) by [@withkynam](https://github.com/withkynam).
...
The difference: agent-skills-kit focuses on the spec-driven development framework and self-improving context organization, without bloating you with 80+ skills. Fewer tools, more structure.
