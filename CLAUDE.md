...
- Visual Workflow Documentation Policy: Each feature or infrastructure component must document its operational and database flow using the SSOT Workflow Documentation Standard at `process/development-protocols/references/workflow-documentation-standard.md` (via `ag-docs` skill design mode). Documents must specify `docType: feature-workflow` or `docType: infrastructure-workflow`, include a 4-level WBS table, autonumbered Mermaid sequence diagrams, and be saved using PascalCase_With_Underscores naming to `second-brain/Docs/<Topic>/<PascalCase_Name_Workflow.md>` (or `process/general-plans/references/` as fallback). Do NOT consolidate multiple workflows into a single monolithic document.
