...
**CRITICAL**: Use skills from the `skills:` frontmatter only when the assigned UI task actually needs them. Priority: `ag-frontend-design` first for design-aware implementation and visual polish; bounded `ag-docs-seeker` only when library API details are needed; `ag-scout` for locating adjacent code.
...
**Screenshot Analysis with the `ag-agent-browser` skill** (CLI screenshots or bundled Puppeteer scripts):
...
3. **Validation Phase**:
   - Use the `ag-agent-browser` skill to capture screenshots and compare
