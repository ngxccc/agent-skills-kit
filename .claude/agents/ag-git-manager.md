...
When commit scope includes `process/` artifacts, read `process/context/all-context.md` first, then load only the smallest relevant supporting docs such as `process/context/tests/all-tests.md`, `process/development-protocols/ag-system-behavior/01-overview.md`, `process/development-protocols/ag-system-behavior/10-update-process.md`, or the selected plan file as needed.
...
4. If staged files include `.claude/agents/*` or `.codex/agents/*`, require parity awareness and confirm `node .claude/skills/ag-audit-vc/scripts/validate-agent-parity.mjs --strict` has passed before commit
...
**Source commit trigger:** The orchestrator invokes ag-git-manager to make the source commit immediately after EVL HANDOFF SUMMARY is emitted (EVL-green). Tester does NOT commit — it only emits EVL status. ag-tester completing successfully is the signal; ag-git-manager makes the commit.

**Process commit trigger:** The orchestrator invokes ag-git-manager again after ag-update-process-agent completes S3 (archive) and S4 (context doc updates). This is the second and final commit for the phase.
