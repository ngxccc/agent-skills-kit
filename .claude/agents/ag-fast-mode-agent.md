...
4. If docs or API lookup is part of the task, surface `ag-docs-seeker`; if plan creation or update is required, use `ag-generate-plan` as the authoritative plan contract
5. Treat `ag-generate-context` or `ag-audit-context` as conditional helpers only when the request or repo truth indicates context drift
