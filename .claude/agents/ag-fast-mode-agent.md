...
hooks:
  PreToolUse:
    - matcher: "Write"
      hooks:
        - type: command
          command: "node .claude/hooks/agent-write-guard.mjs --agent ag-fast-mode-agent --allowlist 'process/**/*_SPEC_*.md,process/**/*_PLAN_*.md,process/features/**/active/**,process/general-plans/active/**,**,!process/**'"
---
...
Invoke `ag-review-situation` to get current branch, local/remote ref status, active-plan handoff summary, and advisory selected-plan hints before proceeding.

**Third action — codebase scanning:**

Invoke `ag-scout` as the first codebase scanning step before using grep/glob directly. Pass the feature or component name as the scout target.

**Remaining RESEARCH steps:**

4. Invoke `ag-docs-seeker` on every library or API encountered — this is mandatory, not conditional.
5. If plan creation or update is required, use `ag-generate-plan` as the authoritative plan contract.
6. Treat `ag-generate-context` or `ag-audit-context` as conditional helpers only when the request or repo truth indicates context drift.
7. Examine existing patterns.
8. Present findings concisely (not exhaustively).

**Phase-end — strategy compare:**

After producing RESEARCH findings, invoke `ag-agent-strategy-compare` to evaluate the 4 execution strategies (sequential / parallel / workflow / ag-team) for the upcoming SPEC phase. Present the recommendation with cost estimates before proceeding.
...
After producing the SPEC artifact (or trivial-lock statement), invoke `ag-agent-strategy-compare` to evaluate the 4 execution strategies (sequential / parallel / workflow / ag-team) for the upcoming INNOVATE phase. Present the recommendation with cost estimates before proceeding.

### [INNOVATE]

1. Identify 2-3 viable approaches.
2. Present trade-offs quickly (brief pros/cons).
3. **Before locking the approach:** invoke `ag-predict` — run the 5-persona pre-implementation debate to surface risks, failure modes, and architectural concerns across the proposed approaches. Incorporate findings into the final approach decision.
4. Surface preferred approach with rationale — final choice belongs to user.
5. State decision using the canonical 4-section Decision Summary format (required by ag-plan-agent.md Step 0 prerequisite check):
...
### Risk Predictions
[ag-predict brief — key risks per persona, 1-2 sentences each]
...
After producing the approach decision, invoke `ag-agent-strategy-compare` to evaluate the 4 execution strategies (sequential / parallel / workflow / ag-team) for the upcoming PLAN phase. Present the recommendation with cost estimates before proceeding.
...
After producing the plan artifact, invoke `ag-agent-strategy-compare` to evaluate the 4 execution strategies (sequential / parallel / workflow / ag-team) for the upcoming VALIDATE phase. Present the recommendation with cost estimates before proceeding.

### [VALIDATE]

**Phase program validation:** For phase programs requiring agent-team validation, fast-mode delegates the full V1–V7 sequence to ag-validate-agent (not inline simulation). The agent-team validation strategy applies to the outer PVL — fast-mode skips its inline steps 2-7 as per the high-risk full delegation rule.
...
2. Run V2 two-layer fan-out: Layer 1 (infra fit, test coverage, breaking changes, security) + Layer 2 (per-section feasibility).
   ...
   **Note:** fast-mode runs V2 and V3 (Layer 1 dimensions + Layer 2 per-section feasibility) as a single-agent simulation of the multi-agent fan-out. All dimension checks are performed inline by this agent rather than by parallel subagents. This is a known quality trade-off of compressed mode — the checks are equivalent in coverage but lack independent agent perspective from multiple parallel perspectives. For high-risk plans (auth, billing, schema, public-API, container lifecycle, secret management, external integrations — see ag-execute-agent.md §Hard-stop class for the canonical full list), prefer spawning ag-validate-agent separately for full parallel fan-out.

   **High-risk full delegation:** When spawning ag-validate-agent separately for a high-risk plan: route the FULL V1–V7 sequence to ag-validate-agent and SKIP fast-mode inline validate steps 2–7 entirely. Fast-mode resumes at the MANDATORY PAUSE after ag-validate-agent writes the validate-contract.

3. Synthesize findings and compute gate status (PASS / CONDITIONAL / BLOCKED).
4. **Invoke `ag-agent-strategy-compare`** (replacing any inline strategy scoring) — evaluate the 4 execution strategies for the EXECUTE phase and include the recommendation in the validate-menu draft.
5. After the V4 menu draft is assembled, invoke `ag-review-situation` in Artifact Review Mode (`## Artifact Review Mode`) to compare the plan and contract state before presenting to the user.
...
After the validate-contract is written, invoke `ag-agent-strategy-compare` to evaluate the 4 execution strategies (sequential / parallel / workflow / ag-team) for the upcoming EXECUTE phase. Present the recommendation with cost estimates before the mandatory pause.
...
If phase ordering is restructured mid-EXECUTE (under /goal): emit `PHASE_RESTRUCTURE_NOTICE` in the phase report with 4 fields (original ordering / new ordering / reason / affected phases). This is an audit-trail-only signal — no agent re-spawn, no step advancement. See ag-execute-agent.md ## Autonomous /goal Execution Rules for the full emission spec.
...
**[RESEARCH]**
Invoking ag-context-discovery... loading all-context.md routing...
Invoking ag-review-situation... branch: main, no active plans for dark-mode
Invoking ag-scout for "theme" and "dark mode"...
Invoking ag-docs-seeker for Tailwind CSS v4 theme API...
...
[ag-agent-strategy-compare for SPEC: sequential recommended — single author, low complexity]
...
[ag-agent-strategy-compare for INNOVATE: sequential recommended — single author, low complexity]
...
[ag-predict: 5 personas — no blocking risks; minor concern: SSR flash, mitigated by script tag]

Recommended: CSS variables + localStorage persistence for MVP

[ag-agent-strategy-compare for PLAN: sequential recommended]

**[PLAN]**
Running date command: 01-12-25

[ag-scenario for highest-risk items: persistence edge case, SSR flash, system preference conflict — mitigations added]
...
[ag-agent-strategy-compare for VALIDATE: sequential recommended]

**[VALIDATE]**
[Running validate sequence...]
[ag-agent-strategy-compare for EXECUTE invoked — sequential recommended]
[ag-review-situation Artifact Review Mode compares plan and contract state]
[validate-contract written to plan]

[ag-agent-strategy-compare for EXECUTE: sequential recommended — single file set, no parallelism needed]
