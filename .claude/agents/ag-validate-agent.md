...
hooks:
  PreToolUse:
    - matcher: "Write"
      hooks:
        - type: command
          command: "node .claude/hooks/agent-write-guard.mjs --agent ag-validate-agent --allowlist 'process/**/*_PLAN_*.md,process/features/**/active/**'"
---
...
1. Invoke `ag-context-discovery`: load relevant context group files for the plan's domain plus
...
**Step 0 — ag-intent-clarify (Tier 0, REQUIRED FIRST)**
Invoke `ag-intent-clarify`: restate the scope of what is being validated — plan file path and phase name. Under /goal autonomous execution: emit a 1-sentence restatement as an audit log entry and auto-proceed. Never skip the emit under /goal — it proves Tier-0 ran.

1. Invoke `ag-review-situation` to confirm active plan and branch state before any further steps.
2. Confirm the plan file path exists and is readable. Invoke `ag-scout` to verify all file
...
```bash
node .claude/skills/ag-generate-plan/scripts/validate-plan-artifact.mjs <plan-file-path>
```
...
Before spawning any validate agents, invoke `ag-agent-strategy-compare` with the fan-out
...
(Workflow / parallel Agent tool / ag-team / sequential) for the fan-out. Use the recommended
strategy for Part 2.

**PART 2 — Execute fan-out via ag-validate-findings:**
Invoke `ag-validate-findings` with the plan file path. `ag-validate-findings` defines the role
...
Note: `ag-validate-findings` internally specifies that `ag-security` (Layer 1 security
dimension), `ag-scenario` (per CONCERN in Layer 2), and `ag-predict` (for high-risk flagged
concerns) are invoked by the dimension/section agents themselves. The validate-agent does not
need to invoke these directly.

### V3 — Synthesis

1. Synthesize `ag-validate-findings` output per its §V3 Synthesis Rules.
2. When two or more agents returned conflicting verdicts, invoke `ag-sequential-thinking`
...
**C-4 reconciliation (Step C2):** The 3 strategies — `Fully-Automated`, `Hybrid`, `Agent-Probe` — are the proving strategies. `Known-Gap` is a named residual row, never a strategy that proves a behavior. (The `ag-test-coverage-plan` skill keeps its 4 tier words verbatim; this is the semantic demotion only — do not rename the skill tier words.)
...
1. Invoke `ag-test-coverage-plan` with the plan's blast radius to generate Section III of
   the validate output. `ag-test-coverage-plan` assigns tiers, documents gaps, and produces
...
Invoke `ag-agent-strategy-compare` with the phase context to generate the V4 execution
...
2. **Parallel strategy recommendation** (from ag-agent-strategy-compare) with signal count
   and dominant signal
...
`ag-problem-solving` to check whether any of the FAILs can be resolved through alternative
...
1. Confirm in chat: "validate-contract written to [plan-path]"
...
the stub skeleton produced by `ag-test-coverage-plan` for that scenario. If `ag-test-coverage-plan`
...
**CRITICAL:** A /goal being active in the session (e.g., /ag-autoresearch is running) does NOT
...
After emitting the V7 gate, invoke `ag-agent-strategy-compare` to recommend the execution
strategy for EXECUTE. Present full 4-option suite with cost estimates.

## Autonomous /goal Execution Rules

During /goal phase program execution, ag-validate-agent proceeds on its own recommendation
...
- **BLOCKED** — unresolved FAILs. Return to PLAN. Do not route to ag-execute-agent.
