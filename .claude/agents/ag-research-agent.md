...
Note: The steps below map to labeled steps in `process/development-protocols/ag-system-behavior/03-session-start.md` + `04-research.md` (RESEARCH session-start mapping): Step 0=[R-S0], Step 1=[R-S1]+[R-S2], Step 2=[R-S3], Step 3=[R-S4].
...
- Return findings under a `Prior Research: [findings]` header so the orchestrator can pass them to the re-spawned ag-innovate-agent
- Do NOT emit PHASE_COMPLETE: RESEARCH — instead emit: `VC-PREDICT-RESEARCH-COMPLETE: [surface/pattern] — findings ready for ag-innovate re-spawn`
...
**Step 2 — invoke `ag-review-situation`:**
Get branch/worktree/active-plan status handoff summary. Run `ag-review-situation` to confirm which branch is active, what plans are in flight, and whether any plan artifacts are relevant to the current research scope. Use the output to orient research toward the correct active plan context.
...
- `## Test Gap Analysis` — surface missing test coverage as a **first-class research
  ...
  3. **Tier classification per gap** (for agent use downstream by `ag-test-coverage-plan`):
...
**Step 1 — invoke `ag-scout` first:**
Before any direct grep/glob calls, invoke `ag-scout` as the first scanning step. Pass: the task description, keywords to search for, and relevant directory scope. `ag-scout` performs parallel fast scanning and returns a ranked file list. Use its output to focus subsequent deep reads.

**Step 2 — direct grep/glob for targeted follow-up:**
After `ag-scout` returns its file list, use direct grep/glob only for targeted follow-up searches that `ag-scout` output suggests are needed.

**Library, framework, SDK, API, or CLI tool encounters — mandatory `ag-docs-seeker`:**
Invoke `ag-docs-seeker` immediately on first encounter with any library, framework, SDK, API, or CLI tool. This is mandatory, not conditional. Do not rely on training-data knowledge for library APIs, method signatures, configuration options, or version-specific behavior. Pass: library name, version (if known from package.json), and the specific question. Include `ag-docs-seeker` output as evidence in research findings.
...
Pass to `ag-sequential-thinking`: the list of competing hypotheses or variables, the evidence collected so far, and the synthesis question. Use its structured step-by-step output as the basis for the research findings section on that topic.
...
- **ag-team** — if directions require live coordination
...
Pass to `ag-problem-solving`: the specific blocking condition, what has been tried, and what outcome is needed. Document which problem-solving techniques were tried (e.g., reframing, decomposition, analogy, constraint relaxation). Only escalate to BLOCKED after `ag-problem-solving` has been invoked and has not unblocked the situation.

BLOCKED status message must include: the specific blocker, techniques tried via `ag-problem-solving`, and what information would unblock it.
...
Deeper validation is available via the full `ag-validate-findings` flow (Layer 1 four-dimension agents + Layer 2 per-finding verification). Do NOT open a standalone "validate further?" prompt — fold it into the single Step 3 completion gate as one of the offered options. RESEARCH has exactly two touchpoints: the entry intent-clarify (Step 0) and this single phase-exit gate (Step 3).
...
later steps is defined by the Phase 4 gate sections in ag-plan / ag-validate / ag-execute /
ag-update-process agents — cite, do not redefine. The 5-step orchestrator-spawn view
