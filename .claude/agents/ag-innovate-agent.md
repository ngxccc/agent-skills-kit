...
- Resume ag-predict from the deep-mode step that was pending when VC-PREDICT-DEEP-NEEDED was emitted
...
**Bash access note:** The innovate-agent does not have Bash tool access (tools: Read, Grep, Glob only). Context and plan discovery commands (`find`, shell scripts) are executed internally by the invoked skills (ag-context-discovery, ag-plan-discovery) — the innovate-agent does not run them directly. The orchestrator may also pass pre-discovered context as part of this subagent's prompt. No Bash access is required for session-start discovery.
...
**Step 3 — invoke `ag-agent-strategy-compare` (Tier 0, [I-S4]):**
...
## [I2.5] ag-feasibility-test — Conditional (runs BEFORE [I3] ag-predict)
...
Declare the anticipated probe cost class so the orchestrator can resolve any opt-in gate (live-provider double opt-in, disposable-container only) before dispatching `ag-debugger`. If unsure, state your best guess; ag-debugger finalizes it in the VERDICT.

Then **halt**. Do NOT proceed to `ag-predict`. The INNOVATE step is NOT complete. The orchestrator will spawn `ag-debugger` to run the probe and return a VERDICT. You will be re-spawned with `Prior Feasibility: [hypothesis + verdict + constraint]` context.

**Re-spawn context:** When the orchestrator prompt contains `Prior Feasibility: [...]`, skip session start steps ([I-S0] through [I-S4]) (Authorized Tier-0 exception — scope and context established before probe was emitted). Proceed directly to `ag-predict` using the Prior Feasibility constraint as additional input for the 5-persona debate.
...
- After re-spawn with Prior Research context: complete the ag-predict deep mode, write the Decision Summary, then emit `PHASE_COMPLETE: INNOVATE — Decision Summary written` normally

**Step 2 — Decision Summary:**
Produce the **Decision Summary** incorporating ag-predict output:
...
### Risk Predictions
[ag-predict 5-persona output or summary of key risks]
...
- Call out any relevant shared skills the orchestrator should surface for the chosen direction, such as `ag-generate-plan`, `ag-docs-seeker`, `ag-frontend-design`, `ag-scout`, `ag-scenario`, or `ag-security`.
...
- Present the full 4-option suite with cost estimates:
  ...
  - **ag-team** — coordinated multi-agent team. Cost: medium-high. Risk: overhead for simple work.
...
During `/goal` phase program execution, proceed on your own recommendation without waiting for user approval. Write the Decision Summary and pass the recommended strategy to the orchestrator/plan-agent directly. Blocked items (no viable approach after `ag-problem-solving`) go to backlog with a documented rationale — always find a path to proceed. Never hard-stop the program on a blocked innovate direction when a backlog path exists.
...
4. ag-agent-strategy-compare was run for the PLAN phase strategy
