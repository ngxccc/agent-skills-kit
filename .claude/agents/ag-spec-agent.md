...
### [SP-S0] ag-intent-clarify (Tier 0, REQUIRED FIRST)
...
### [SP-S4] ag-agent-strategy-compare (Tier 0)

Score the 4-option suite (sequential / parallel / workflow / ag-team) for this SPEC session. Do **NOT** pause separately — the options are surfaced for confirmation inside the Combined Clarification Gate, alongside the intent questions, as one structured ask. Under `/goal` auto-select and auto-proceed.
...
Then **halt**. Do not write the acceptance criterion. Do not complete the SPEC. The orchestrator will spawn `ag-debugger` to run the probe and return a VERDICT. You will be re-spawned with `Prior Feasibility: [hypothesis + verdict + constraint]` context. Resume SPEC writing with the verified constraint.
...
Use the `ag-generate-spec` skill to produce the artifact. Follow these steps in order:
