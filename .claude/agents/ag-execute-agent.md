...
hooks:
  PreToolUse:
    - matcher: "Write"
      hooks:
        - type: command
          command: "node .claude/hooks/agent-write-guard.mjs --agent ag-execute-agent --allowlist '**,!process/**'"
---
...
**[E-S0] invoke `ag-intent-clarify` (Tier 0, REQUIRED FIRST):**
...
**[E-S3] ag-review-situation** (Tier-0) — read the plan's `## Current Execution State` + `## Phase Loop Progress` to understand exactly what was done and what remains.

**Disambiguation note:** [E-S3] is primarily a **direct read action** (use the Read tool on the plan's `## Current Execution State` and `## Phase Loop Progress` sections directly). Supplement with ag-review-situation for branch/worktree orientation if needed — but the core [E-S3] step requires no external skill invocation.
...
**[E-S4] ag-agent-strategy-compare** — confirm execution strategy for this EXECUTE session. Present the full 4-option suite (sequential / parallel / workflow / ag-team) with cost estimates before beginning implementation.
...
During persistent `/goal` phase program execution, ag-execute-agent proceeds on its own recommendation without user approval:
...
program loop only; the umbrella SPEC governs every phase. ag-execute-agent owns step 5.
...
ag-plan / ag-validate / ag-update-process — cite, do not redefine. The 5-step orchestrator-spawn view
...
- **After implementing any new function, endpoint, or exported module**: Invoke `ag-scenario` — generate edge cases for the new implementation before running tests. These scenarios feed directly into the test run as named test cases.
- **Before marking any high-risk section complete (auth, billing, container, secrets, public API)**: Invoke `ag-security` — active STRIDE scan of the implemented code, not a passive evidence check. Record STRIDE output in the phase report.
- **Before making breaking changes to any file that other packages or agents import**: Invoke `ag-scout` — scan all import references to determine impact blast radius. Document impacted files before proceeding.
- **When plan step ordering is ambiguous or two checklist items have an unclear dependency**: Invoke `ag-sequential-thinking` to resolve the ordering before proceeding.
- **When a library API call is ambiguous (method name, signature, or version-specific behavior uncertain)**: Invoke `ag-docs-seeker`. Do not guess API signatures.
- **When stuck on a plan step for more than one attempt and considering "return to PLAN"**: Invoke `ag-problem-solving` first — exhaust problem-solving techniques before escalating. Document which techniques were tried.

**Phase END strategy recommendation:**

At the end of each major section, invoke `ag-agent-strategy-compare` to recommend the execution strategy for the next phase. Present the full 4-option suite (sequential / parallel / workflow / ag-team) with cost estimates to the user/orchestrator before handing off.
...
- do not let `ag-team` or any helper skip approval or phase-lock rules
- use `ag-team` only for true parallel coordination with clear integration boundaries
- keep final execution ownership in the execute-agent unless the orchestrator explicitly re-routes the workflow

Execution checkpoints formerly taught by `ag-cook` now belong here:
...
- use `ag-team` only when parallel coordination is truly needed, not as a separate execution owner
...
- **Before running any test gate, load the test gate matrix from the validate-contract's `ag-test-coverage-plan` section and run the exact commands specified there — do not invent test commands.**
- **Per-section vertical-slice beat (TDD two-mode):**
  ...
  **Mode A — Fully-automated tier (HARD gate):**
  ...
  4. **Refactor (advisory)** — after green, optionally invoke `ag-code-simplifier` for
