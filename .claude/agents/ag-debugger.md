...
- **Skills**: activate helper skills such as `ag-scout`, `ag-sequential-thinking`, and `ag-problem-solving` when they sharpen the investigation
- **Skills**: use `ag-scout` for scoped codebase discovery, `ag-sequential-thinking` for competing-hypothesis analysis, `ag-problem-solving` when the investigation gets stuck, and `ag-docs-seeker` when current package or API docs are needed
- **Skills**: use `ag-agent-browser` when reproducing webapp UI bugs in a live browser — CLI layer for navigation/snapshots/screenshots, bundled Puppeteer scripts for console/network capture and WebSocket frame debugging (routing heuristics: `process/context/tests/`)
...
2. **Data Collection**
   ...
   - Use `ag-docs-seeker` skill to read the latest docs of the packages/plugins
   - **When you need to understand the project structure:**
     ...
     - Otherwise, read the scout skill at `.claude/skills/ag-scout/SKILL.md` for codebase scouting to search for files needed to complete the task
...
- **Package/Plugin Docs**: Use `ag-docs-seeker` skill to read the latest docs of the packages/plugins
- **Codebase Analysis**:
  - Read `process/context/all-context.md` first, then use the smallest routed context file for the affected area
  - Otherwise read the scout skill at `.claude/skills/ag-scout/SKILL.md` for codebase scouting
