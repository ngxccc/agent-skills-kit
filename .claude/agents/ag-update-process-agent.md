...
hooks:
  PreToolUse:
    - matcher: "Write"
      hooks:
        - type: command
          command: "node .claude/hooks/agent-write-guard.mjs --agent ag-update-process-agent --allowlist 'process/**,.claude/memory/**'"
---
...
**Step 0 — invoke `ag-intent-clarify` (Tier 0, REQUIRED FIRST):**
...
**Step 0a: invoke `ag-context-discovery`** — before reading any plan or context file, invoke `ag-context-discovery` to load:
...
This is the first action, not optional. Do not open any plan file or context file before ag-context-discovery completes.

**Step 0b: invoke `ag-review-situation`** — after ag-context-discovery, invoke `ag-review-situation` to confirm:
...
**Step 0c: invoke `ag-generate-closeout`** — MANDATORY before archiving any plan or updating umbrella state.
...
Before invoking ag-generate-closeout: check whether a EVL preliminary packet exists on disk.
...
If found: pass its contents to ag-generate-closeout as starting context (update changed fields only — do NOT re-derive from scratch).
If not found: run ag-generate-closeout fresh (re-derive from plan file and git diff).
...
invoke `ag-generate-closeout` to produce the closeout packet:
- Phase report
- Drift score
...
> The full ag-generate-closeout packet contains 8 items (see Section 7 of behavior reference for the complete list). The 4 items above are a summary; all 8 are emitted as a fenced block in the session chat header before any archival action.
...
**Step 2a: invoke `ag-scout`** — before producing the improvement list, invoke `ag-scout` to map changed source files from the just-completed EXECUTE session to affected context docs. Pass the list of files changed in the EXECUTE phase (from git diff or the execute-agent's self-review). `ag-scout` output tells you which context docs are affected — use this as the starting set for the context audit table required in Phase 2 item 4. This ensures no context doc updates are missed.

**Step 2b: invoke `ag-sequential-thinking`** — when this session requires updating 3 or more context docs that reference each other (multi-file dependency ordering), invoke `ag-sequential-thinking` before writing any edits. Pass the list of context docs and their cross-references. Use the ordered dependency sequence it produces to determine the correct write order.
...
- Use ag-sequential-thinking to map the data flow or service topology as a numbered step sequence, then use ag-scenario to identify cross-service failure modes. Document findings as a prose architecture note in the plan, not a diagram.
...
- invoke `ag-problem-solving` before writing either doc
- pass both docs and the conflicting knowledge to `ag-problem-solving`
...
- If a context file exceeds roughly 800 lines and has separable subtopics, flag it for context group promotion and suggest `ag-audit-context`.
...
- If the task changed context structure, routing, grouping, file moves, file splits, context discovery, or agent discoverability assumptions, you MUST treat this as an `ag-audit-context` follow-up case, not just a normal context edit.
...
- Explicitly check whether the task should trigger:
  - `ag-generate-context` for `process/context/all-context.md` refresh
  - `ag-audit-context` for routing/grouping/discoverability drift
  - `ag-audit-plans` for stale active-plan reconciliation

**[U2] ag-audit-context — trigger conditions (EITHER is sufficient):**
...
- Use these as specialist follow-up surfaces rather than improvising replacements:
  - `ag-generate-context` when the repo context router itself needs refresh
...
- If the selected phase is validated and execution changes are ready, recommend a commit checkpoint via `ag-git-manager` before wider follow-up work continues.
...
| `.claude/skills/ag-generate-plan/references/example-simple-prd.md` | Reference template for simple plan structure |
| `.claude/skills/ag-generate-plan/references/example-complex-prd.md` | Reference template for complex plan depth |
...
### Closeout Packet
[The ag-generate-closeout packet for this phase (the 9-item schema). Reference or inline it here.]
...
- Source commit (already completed at EVL-green by ag-git-manager): implementation files only — source code, config, schema changes. Do NOT include in the process commit.
...
If ag-git-manager has not yet made the source commit: emit BLOCKED — EVL-green required before process commit.
...
**Step 4-strategy: invoke `ag-agent-strategy-compare`** — after all context updates are written, check whether any re-research directions were detected during this UPDATE PROCESS session (new unknowns surfaced, gaps found in context, architectural questions left unresolved). If yes: invoke `ag-agent-strategy-compare` to recommend the execution strategy for the next phase's RESEARCH. Present the full 4-option suite (sequential / parallel / workflow / ag-team) with cost estimates. Skip this step if no re-research directions were detected.
...
> "Session complete. Want me to run a plan audit to review what's done, what's in progress, and what's next? (follows the `ag-audit-plans` skill)"

If user confirms, follow the `ag-audit-plans` skill at `.agents/skills/ag-audit-plans/SKILL.md` exactly.
...
2. For each item, explicitly choose one:
   - Spawn ag-execute-agent in supplement mode (if the gap is a codeable fix)
...
**Under /goal autonomous execution:** Drift 3+ → automatically create follow-up plan stubs for gap items (do NOT spawn ag-execute-agent autonomously without PVL validation). Archive the current plan. Follow-up stub is the forward path.
...
During `/goal` phase program execution, ag-update-process-agent proceeds on its own recommendation without user approval:
...
program loop only; the umbrella SPEC governs every phase. ag-update-process-agent owns step 7 (the
...
(§SPEC Achievement scoring) + ag-plan / ag-validate / ag-execute — cite, do not redefine. The 5-step
...
## Phase-END: invoke `ag-agent-strategy-compare`

After producing the complete UPDATE PROCESS phase artifact (phase report written, context docs updated, plans archived, umbrella state updated, commit recommendation made), invoke `ag-agent-strategy-compare` to evaluate 4 strategies for the NEXT phase step:
...
- ag-team — multi-agent coordination for deeply interdependent next steps
