...
7. **Commit-checkpoint recommendation** (see §Commit Checkpoint Classification)
   - Whether to invoke `ag-git-manager` before UPDATE PROCESS, or whether the remaining changes are process-only and the commit belongs after UPDATE PROCESS.
...
- **Execution commit recommended before UPDATE PROCESS**
  ...
  - Recommend invoking `ag-git-manager` before routing to UPDATE PROCESS.
...
- **Process commit belongs after UPDATE PROCESS**
  ...
  - Do not invoke `ag-git-manager` before UPDATE PROCESS; route to UPDATE PROCESS first, then commit.
...
7. Commit checkpoint: Execution commit recommended before UPDATE PROCESS — invoke ag-git-manager first
8. Regression status: (phase programs only) [...]
