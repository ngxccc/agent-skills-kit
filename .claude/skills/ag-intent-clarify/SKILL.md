...
Scores a user request's ambiguity and — when Tier 2 triggers — produces a structured, multi-dimension clarification suite with option-rich questions rather than open-ended prompts. Uses ag-scenario, ag-predict, and ag-sequential-thinking style reasoning to generate questions, not just to answer them.
...
5. Pure information question / resuming active plan (lowest priority — treated as Tier 0, auto-route to ag-research-agent or answer directly)
...
1. ag-review-situation — current branch, worktrees, active plans, uncommitted changes
2. ag-scout — scan codebase for files/modules relevant to: [keywords from request]
3. ag-sequential-thinking — map the decision dimensions: what must be decided first? what is downstream?
4. ag-scenario — for each plausible interpretation: what are the top 3 failure modes?
5. ag-predict — 5-persona debate: senior dev / PM / security reviewer / QA / end user — what does each want to know before starting?
