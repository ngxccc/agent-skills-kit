...
const rfcDir = fs.existsSync(primaryRfcDir) ? primaryRfcDir : (fs.existsSync(fallbackRfcDir) ? fallbackRfcDir : primaryRfcDir);
let failures = 0;
...
if (files.length === 0) {
  console.log(`${colors.yellow}Warning: No RFC files found in docs/rfc or second-brain/Docs/RFCs.${colors.reset}`);
  process.exit(0);
}

console.log(`${colors.cyan}Auditing ${files.length} Request for Comments (RFCs)...${colors.reset}\n`);
...
for (const file of files) {
  ...
  if (!fileMatch) {
    logError(file, "Filename must match standard 4-digit prefix pattern, e.g., '0001-proposal-title.md'");
    continue;
  }
  ...
  if (!h1Match) {
    ...
    if (headingNum !== fileNum) {
      logError(file, `Heading RFC number (${headingNum}) does not match filename prefix (${fileNum})`);
    }
  }
  
  // 3. Check Metadata fields (Status)
  const statusRegex = /^Status:\s*(Draft|Under Review|Approved|Rejected|Superseded.*)/im;
  if (!statusRegex.test(content)) {
    logError(file, "File must include valid 'Status:' metadata (Draft, Under Review, Approved, Rejected, Superseded)");
  }
...
console.log("\n----------------------------------------");
if (failures > 0) {
  console.error(`${colors.red}${colors.bold}Audit failed with ${failures} error(s).${colors.reset}`);
  process.exit(1);
} else {
  console.log(`${colors.green}${colors.bold}All RFC files passed validation cleanly.${colors.reset}`);
  process.exit(0);
}
