...
- **PVL:** the ORCHESTRATOR invokes it when ag-validate-agent returns a first-pass CONDITIONAL/BLOCKED verdict (validate-fix loops are needed)
...
Subagents (ag-validate-agent, ag-tester, ag-plan-agent, ag-execute-agent) are fire-and-forget: they emit a verdict and terminate. They cannot invoke this skill on the orchestrator's behalf, cannot loop themselves, and cannot spawn each other.
...
When ag-autoresearch is the bookkeeper for a PVL (plan-validate-fix loop):

- `domain: plan`, `corpus:` = the plan .md file
- **Research step** = ag-validate-agent runs V1–V3 gates; autoresearch does NOT run these itself. The validate step itself fans out in parallel via `ag-validate-findings` (Layer 1 dimension agents + Layer 2 feasibility agents) — that parallelism is owned by ag-validate-agent.
...
When ag-autoresearch is the bookkeeper for an EVL (execute-validate-fix loop):

- `domain: tests`, `verify:` = validate-contract gate commands (read from contract — never invented)
- **Research step** = a SPAWNED ag-tester agent runs the validate-contract fully-automated gate commands (ag-tester may run independent gate groups in parallel). The orchestrator NEVER runs gate commands in its own shell — a gate result not produced by a ag-tester spawn does not count as an EVL confirmation, even if green.
- **Gap signal** = failing gate (non-zero exit)
- **Fix step (parallel)** = when multiple independent gates fail across non-overlapping file groups, the orchestrator spawns **multiple parallel execute-fix agents** (ag-execute-agent in supplement mode), one per failing gate / file group, partitioned so no two agents edit the same file. Each fixer is scoped to exactly its failing gate — no scope expansion. When failing gates share files or a single root cause, fall back to a single execute-fix agent. The fix is ALWAYS a ag-execute-agent spawn — the orchestrator never edits source files itself, no matter how small the fix.
...
Run `ag-autoresearch:evals {task_folder}/results.tsv` to analyze trends and get a plateau/recommendation report.
