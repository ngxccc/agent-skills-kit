...
// package.json
{
  ...
  "typesVersions": {
    // legacy TS subpath types resolution
    "*": {
...
3. **Subpath exports must be explicit.** Once you have an `exports` field, _unlisted_ subpaths are inaccessible — `import 'acme-sdk/internal/util'` errors. This is the feature, not a bug.
...
- No existing TypeScript skill in this repo covers library type publishing. Companion to `[[dsl-narrow-api-surface]]`: that rule controls _what_ a library exports, this rule controls _how_ those exports resolve across consumer toolchains.
