...
type Protocol<S extends string> = S extends `${infer P}://${string}`
  ? P
  : never;
...
Eager vs lazy: template literals match greedily at the _first_ delimiter from the left. To match at the _last_ delimiter, switch direction with `${string}` as the leading wildcard and capture the right side: `` `${string}/${infer Last}` ``.
...
- Provides the _pattern primitives_ used by `[[dsl-route-param-inference]]`, `[[dsl-type-safe-object-paths]]`, and `[[tlp-type-level-string-algorithms]]`. Think of it as the lexer; the other rules are parsers built on top.
