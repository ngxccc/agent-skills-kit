...
Triggered when ag-setup calls this skill and passes an `approved_groups` list (output of Round 1 Subagent C in ag-setup's STUDY phase).
...
- Write `all-context.md` (Subagent E responsibility in ag-setup remains — this mode writes per-group files only when called for that purpose; see ag-setup Round 2 delegation note).
...
- Never delete existing group files — deletion is `ag-audit-context`'s domain.
...
4. Never remove or rename existing group files — deletion is `ag-audit-context`'s domain.
...
```bash
node .claude/skills/ag-context-discovery/scripts/discover-context.mjs --emit-routing
...
- `.claude/skills/ag-generate-plan/references/example-complex-prd.md` for plan/PRD depth expectations.
