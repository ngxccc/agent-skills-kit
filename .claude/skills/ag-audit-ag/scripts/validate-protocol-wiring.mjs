...
if (fs.existsSync(agentsDir)) {
  for (const entry of fs.readdirSync(agentsDir, { withFileTypes: true })) {
    ...
    const refs = [
      ...text.matchAll(/process\/development-protocols\/([a-z0-9_-]+\.md)/g),
    ];
    for (const ref of refs) {
      const referencedFile = ref[1];
      if (!protocolFiles.includes(referencedFile)) {
        warn(
          `${agentFile} references process/development-protocols/${referencedFile} which does not exist on disk`,
        );
      }
    }
  }
...
const updateProcessAgent = '.claude/agents/ag-update-process-agent.md';
