#!/usr/bin/env node
/**
 * Validate that every backtick-wrapped ag-* token in agent bodies refers to a
 ...
 *
 * Exit 0: all ag-* backtick tokens are registered skills or known agent names.
...
// ---------------------------------------------------------------------------
// C1 allowlist: backtick-wrapped tokens that match ^ag-[a-z-]+$ but are
// agent names (not skills) and must NOT be flagged as unregistered skills.
//
// Specified in validate-contract C1: ag-team, ag-research, ag-plan, ag-cook,
// ag-git-manager.
//
// Additional tokens discovered during Phase 2 real-agent scan (documented,
// not silent expansion):
//   ag-code-simplifier — registered agent (.claude/agents/ag-code-simplifier.md),
//                        same class as ag-git-manager; invoked from ag-execute-agent
//   ag-code-review     — legacy agent name referenced in historical prose
//                        ("formerly taught by `ag-code-review`") in ag-code-reviewer.md
//   ag-debugger        — registered agent (.claude/agents/ag-debugger.md),
...
const CATALOG_CANDIDATES = [
  ".claude/skills/ag-context-discovery/generated-skills-catalog.json",
...
//   2. Find all backtick-wrapped tokens matching /`(ag-[a-z-]+)`/g.
//   3. For each token matching ^ag-[a-z-]+$:
//      a. Skip if in AGENT_NAME_TOKENS (allowlist).
//      b. Fail if NOT in registeredSkills.
// ---------------------------------------------------------------------------
const VC_TOKEN_RE = /`(ag-[a-z-]+)`/g;
...
for (const absPath of agentFiles) {
  ...
  for (let i = 0; i < bodyLines.length; i++) {
    ...
    for (const match of line.matchAll(VC_TOKEN_RE)) {
      ...
      // Only check tokens that strictly match ^ag-[a-z-]+$ (no slash, no dot)
      if (!/^ag-[a-z-]+$/.test(token)) continue;
