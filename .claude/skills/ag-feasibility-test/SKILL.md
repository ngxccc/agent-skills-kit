...
## Boundary vs ag-test-coverage-plan
...
- When ag-spec-agent, ag-innovate-agent, or a ag-validate-agent Layer 2 dimension agent emits `VC-FEASIBILITY-PROBE-NEEDED`
- When a mechanism cannot be verified from source code alone
- One-shot: not an iterative loop (use ag-autoresearch for iteration)

## Skill Executor

Always executed by ag-debugger (via the `VC-FEASIBILITY-PROBE-NEEDED` signal routing
