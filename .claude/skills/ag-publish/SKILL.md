...
Push harness improvements from the current development repo to the remote kit repository (`agent-skills-kit`). This is the **maintainer** counterpart to `ag-update`.
...
- Local checkout of the kit repo (`git clone git@github.com:withkynam/agent-skills-kit.git`)
...
12. On confirm:
    ...
    - Update `ag-manifest.json`: bump `version` field per the chosen bump type. **No other manifest changes needed** -- glob patterns are stable, new files are automatically included.
...
13. Verify no project-specific content leaked into the kit repo. This is a
    ...
    **Keep the existing narrow `CLAUDE.md`/`AGENTS.md` grep** (this stays as-is on
    ...
    The standing `validate-kit-portability.mjs` validator (run by `ag-audit-ag`)
...
Files changed: 4
Remote:        git@github.com:withkynam/agent-skills-kit.git
Tag:           v2.2.0
