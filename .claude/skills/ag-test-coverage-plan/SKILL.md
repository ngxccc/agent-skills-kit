...
## Boundary vs ag-feasibility-test
...
1. Invoke `ag-context-discovery` to load the relevant context group files.
...
**Hard stop (mirrors `ag-plan-agent` TIER_ASSIGNMENTS_BLOCKED):** if the `all-tests.md` routing
...
Clarification note: `ag-test-coverage-plan` retains its exhaustive behavior-inventory framing —
