...
function logError(file, message) {
  console.error(`${colors.bold}${colors.red}FAIL:${colors.reset} [${file}] ${message}`);
  failures++;
}
...
for (const group of skillDirs) {
  ...
  const dirs = fs.readdirSync(group.path).filter((d) =>
    fs.statSync(path.join(group.path, d)).isDirectory()
  );
  ...
  for (const dir of dirs) {
    ...
    if (!nameMatch) {
      ...
      if (name !== dir) {
        logError(skillFile, `Frontmatter name '${name}' must match folder name '${dir}'.`);
      }
    }
    ...
    if (!descMatch) {
      ...
      if (!/\b(use|when|for)\b/i.test(desc)) {
        logError(skillFile, "Description missing trigger language: must contain 'use', 'when', or 'for'.");
      }
      if (desc.length < 24) {
        logError(skillFile, "Description is too short (must be at least 24 characters).");
      }
    }
