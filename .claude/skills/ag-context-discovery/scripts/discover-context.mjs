#!/usr/bin/env node
// Auto-discovery for ag-context-discovery.
...
function contextDocsWithFm() {
  return contextFiles.filter(
    (d) => d.path.endsWith(".md") && d.fm && d.fm.hasFrontmatter,
  );
}
...
function buildRoutingBlock() {
  ...
  const rootRows = [
    ...
    ...eps.map(
      (d) => `| \`${d.path}\` | ${d.fm.readWhen || d.fm.description || "—"} |`,
    ),
  ];
  const groupRows = eps.length
    ? eps.map(
        (d) =>
          `| \`${groupOf(d.path)}/\` | \`${d.path}\` | ${d.fm.description || "—"} |`,
      )
    : [
...
if (emitRouting || checkRouting) {
  if (!fs.existsSync(path.join(root, ROUTER))) {
    process.stderr.write(
      `[bare-kit mode] ${ROUTER} absent — nothing to ${emitRouting ? "emit" : "check"}.\n`,
    );
    process.exit(0);
  }
  ...
  if (existing === null) {
    console.error(
      `${ROUTER} has no <!-- GENERATED:routing --> markers; cannot manage routing block.`,
    );
    process.exit(2);
  }
  ...
  if (checkRouting) {
    if (existing.trim() !== fresh.trim()) {
      console.error(
        `${ROUTER} routing block is STALE — run discover-context.mjs --emit-routing to rebuild.`,
      );
      process.exit(1);
    }
  ...
  console.log(
    `Rewrote routing block in ${ROUTER} (${groupEntrypoints().length} group entrypoint(s)).`,
  );
  process.exit(0);
}

if (matchQuery) {
  const tokens = (matchQuery.toLowerCase().match(/[a-z0-9]+/g) || []).filter(
    (t) => t.length > 2,
  );
      ...
      JSON.stringify(
        {
          ...
          matches: scored.map((x) => ({
            ...
            hits: x.hits,
            name: x.d.fm.name,
            description: x.d.fm.description,
          })),
  ...
  if (scored.length === 0)
    out.push(
      "  (no keyword matches — fall back to all-context.md routing table)",
    );
  for (const x of scored)
    out.push(
      `  [${x.score}] ${x.d.path} — ${x.d.fm.description ?? "—"}  (hits: ${x.hits.join(", ")})`,
    );
  if (siblings.length) {
    out.push("");
    ...
    for (const s of siblings)
      out.push(
        `  ${s.sib.path} — ${s.sib.fm.description ?? "—"}  (related via ${s.from})`,
      );
  }
...
if (asJson) {
  const out = {
    ...
    context: contextFiles.map((d) => ({
      path: d.path,
      ...(d.fm ?? { hasFrontmatter: false }),
    })),
    protocols: protocolFiles.map((d) => ({
      path: d.path,
      ...(d.fm ?? { hasFrontmatter: false }),
    })),
    features: featureFiles.map((d) => ({
      path: d.path,
      ...(d.fm ?? { hasFrontmatter: false }),
    })),
    generalPlansActive: generalPlanFiles.map((d) => ({
      path: d.path,
      ...(d.fm ?? { hasFrontmatter: false }),
    })),
  };
