...
- Every subagent FIRST ACTION: run ag-context-discovery (load context group files +
  process/context/tests/all-tests.md routing chain) AND ag-plan-discovery (same-feature full
  depth active/backlog/completed/reports/refs + other features active-only + general-plans active)
- Every phase-END: invoke ag-agent-strategy-compare for next step strategy recommendation
...
- Validate-contract is placeholder and ag-validate-agent cannot run
...
VALIDATE CONTRACT: Per-phase contracts written by ag-validate-agent into each phase plan before EXECUTE.

START: Phase {N}, loop step RESEARCH (pending). Spawn ag-research-agent for Phase {N}.
```
...
4. **PVL** — spawn ag-validate-agent: full V1-V7; validate-contract written per `.claude/skills/ag-validate-findings/references/example-validate-output.md` format (Status / Gate / Plan updates applied / Execute-agent instructions / Test gates / High-risk pack / Backlog artifacts / Known gaps / Accepted by)
5. **EXECUTE** — spawn ag-execute-agent per approved plan and validate-contract
6. **EVL** — spawn ag-tester: run phase test gates to green; register follow-up stubs; write EVL HANDOFF SUMMARY
...
**PVL is NEVER skipped.** A placeholder `## Validate Contract` = blocked. Do not spawn execute-agent while the Validate Contract section reads "(placeholder — ag-validate-agent writes this section before EXECUTE)".
...
- Validate-contract status: pending (ag-validate-agent writes per-phase)
...
- Next action: {e.g. "Spawn ag-research-agent for Phase 1"}
