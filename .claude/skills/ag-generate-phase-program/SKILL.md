---
name: ag-generate-phase-program
...
**Step 1 — invoke `ag-agent-strategy-compare`.**
...
**Per-phase strategy invocation (mandatory):** For each individual phase during scaffold, invoke `ag-agent-strategy-compare` with that phase's scope before writing that phase's plan stub — not once at the program level. Each phase may have a different recommended execution strategy.
...
- **PER-PHASE LOOP must state:**
  ...
  - Every subagent first action: ag-context-discovery + ag-plan-discovery (once available)
  - Every phase-END: invoke ag-agent-strategy-compare
...
8. **Commit checkpoint**
   - if the phase produced implementation changes, recommend `ag-git-manager` for a logical
