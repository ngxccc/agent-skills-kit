...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === '--summary' && i + 1 < args.length) params.summary = args[++i];
    else if (arg === '--body' && i + 1 < args.length) params.body = args[++i];
    else if (arg === '--body-file' && i + 1 < args.length) params.bodyFile = args[++i];
    else if (arg === '--plan' && i + 1 < args.length) params.plan = args[++i];
    else if (arg === '--issue' && i + 1 < args.length) params.issue = args[++i];
    else if (arg === '--assignee' && i + 1 < args.length) params.assignee.push(...args[++i].split(','));
    else if (arg === '--label' && i + 1 < args.length) params.label.push(...args[++i].split(','));
    else if (arg === '--project' && i + 1 < args.length) params.project = args[++i];
    else if (arg === '--milestone' && i + 1 < args.length) params.milestone = args[++i];
...
const CONVENTIONAL_TYPES = ['feat', 'fix', 'refactor', 'perf', 'test', 'docs', 'chore', 'style', 'ci', 'build'];
...
function validateTitle(params) {
  ...
  if (!title) {
    throw new Error('PR Title is required. Use --title "<type>(<scope>): <summary>"');
  }

  // Check for emojis or non-ASCII icon characters
  const emojiRegex = /[\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]/u;
  if (emojiRegex.test(title)) {
    throw new Error(`PR title "${title}" contains emojis/icons, which violate enterprise repository standards.`);
  }

  const conventionalRegex = /^(feat|fix|refactor|perf|test|docs|chore|style|ci|build)(\([a-z0-9_.\/-]+\))?!?: .+/;
  if (!conventionalRegex.test(title)) {
    console.warn(`[WARNING] PR title "${title}" does not strictly match conventional commit format: <type>(<scope>): <summary>`);
  }
...
function main() {
  try {
    ...
    const labels = validateAndFormatLabels(params.label, params.type, params.scope);
