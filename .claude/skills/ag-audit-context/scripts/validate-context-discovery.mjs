...
for (const skill of [
  ...
  if (exists(file)) {
    ...
    // YAML name uses ag- prefix matching folder name convention
    if (fm.name !== skill)
      fail(
...
// require a populated context tree that ag-setup creates after install.
...
if (!bareKitMode) {
  ...
  for (const doc of contextDocs) {
    ...
    if (keywords.length === 0) {
      ...
      warn(
        `${doc} has empty/missing 'keywords' frontmatter (recommended for keyword routing; backfill at UPDATE-PROCESS — see ag-context-discovery)`,
      );
    }
    ...
    for (const slug of related) {
      if (!contextDocNames.has(slug)) {
        fail(
          `${doc} 'related' references ${slug} which resolves to no context doc (dangling cross-link)`,
        );
      }
    }
  }
  ...
  if (routerText.includes("<!-- GENERATED:routing -->")) {
    try {
      execSync(
        `node ${JSON.stringify(path.join(root, ".claude/skills/ag-context-discovery/scripts/discover-context.mjs"))} --check-routing`,
        { cwd: root, stdio: ["pipe", "pipe", "pipe"] },
      );
    } catch {
      fail(
        `${router} GENERATED:routing block is stale — run discover-context.mjs --emit-routing to rebuild`,
      );
    }
  }
  ...
  for (const file of [
    "AGENTS.md",
    "CLAUDE.md",
    ".claude/agents/ag-update-process-agent.md",
    ".codex/agents/ag-update-process-agent.toml",
    ".claude/agents/ag-research-agent.md",
    ".codex/agents/ag-research-agent.toml",
    ".claude/skills/ag-generate-context/SKILL.md",
    ".claude/skills/ag-generate-context/references/generate-context.md",
  ]) {
...
if (!bareKitMode) {
  ...
  const contextDocsForCheck = walk("process/context", (rel) =>
    rel.endsWith(".md"),
  ).sort();
  if (
    contextDocsForCheck.length > 0 &&
    !routerText.includes("Context Group Lifecycle")
  ) {
...
const result = {
  checkedContextDocs: bareKitMode
    ? 0
    : walk("process/context", (rel) => rel.endsWith(".md")).length,
