...
for (let index = 0; index < inventory.length; index += 1) {
  for (let cursor = index + 1; cursor < inventory.length; cursor += 1) {
    ...
    if (left.normalized === right.normalized) {
      warn(
        `skills ${left.folder} and ${right.folder} normalize to the same name (${left.normalized})`,
      );
      continue;
    }
    if (
      ...
      warn(
        `skills ${left.folder} and ${right.folder} may be confusable by prefix (${left.primary} vs ${right.primary})`,
      );
      continue;
    }
    if (levenshtein(left.normalized, right.normalized) === 1) {
      warn(
        `skills ${left.folder} and ${right.folder} differ by edit distance 1 (${left.primary} vs ${right.primary})`,
      );
    }
  }
}
