...
const catalog = {
  ...
  skills: inventory.map((skill) => ({
    ...
    mentionedSkills: extractSkillMentions(skill.text, aliasesBySkill).filter(
...
if (shouldCheck) {
  ...
  if (!fs.existsSync(target)) {
    console.error(
      `${outputPath} missing; run generate-skills-catalog.mjs --write first`,
    );
    process.exit(1);
  }
  ...
  if (existing !== next) {
    console.error(
      `${outputPath} is stale; regenerate with generate-skills-catalog.mjs --write`,
    );
    process.exit(1);
  }
}
