...
### With `ag-agent-browser` Skill

Use `ag-agent-browser` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
python scripts/gemini_batch_process.py \
  ...
  --output process/features/<feature>/active/{task-folder}/extracted-design-guidelines.md \
  --model gemini-2.5-flash
```
