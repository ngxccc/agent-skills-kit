...
SKILL_DIR=""
if [ -d ".claude/skills/ag-agent-browser/scripts" ]; then
  SKILL_DIR=".claude/skills/ag-agent-browser/scripts"
elif [ -d "$HOME/.claude/skills/ag-agent-browser/scripts" ]; then
...
cd $SKILL_DIR/.claude/skills/ag-agent-browser/scripts
...
**Important**: Always save screenshots to the `/tmp/` directory (e.g. `/tmp/screenshot.png`) to avoid storing screenshot artifacts inside the codebase or skill directory.

```bash
node screenshot.js --output /tmp/screenshot.png [--url https://example.com] [--full-page true] [--selector .element] [--max-size 5] [--no-compress]
```
