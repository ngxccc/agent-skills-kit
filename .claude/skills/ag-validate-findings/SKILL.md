...
- VALIDATE V2–V3: invoked by `ag-validate-agent` after V1 pre-check passes
...
This skill is **STRATEGY-AGNOSTIC**. It defines the role specs, prompts, and output schemas for each dimension/section agent. It does NOT determine how those agents are executed (Workflow vs parallel Agent tool vs ag-team).

The execution method is determined by `ag-agent-strategy-compare`, which must be invoked BEFORE `ag-validate-findings` is called. `ag-validate-findings` outputs agent role definitions; the caller executes them using the recommended strategy.
...
**Note:** For each CONCERN found, INVOKE `ag-scenario`. For high-risk flagged concerns, INVOKE `ag-predict`.

---

## V3 Synthesis Rules

The ag-validate-agent (or orchestrator) synthesizes all Layer 1 and Layer 2 outputs:
...
**Note:** Invoke `ag-sequential-thinking` at this synthesis step for contradiction ranking.
