...
`ag-validate-agent` must produce. It shows every section at maximum detail so
...
A fundamental design problem exists that requires rethinking the approach — not just fixing plan text. [FAILs only, or user-identified architecture issue.] Route back to ag-plan-agent. VALIDATE re-runs from V1 after plan is revised.
