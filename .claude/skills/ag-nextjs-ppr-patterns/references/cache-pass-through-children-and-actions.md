...
The model assumes `'use cache'` on a wrapper caches _everything_ rendered inside it, so it avoids wrapping any dynamic UI — or it tries to call a passed-in Server Action inside the cached body. Neither is right: a cached component can receive `children` (and Server Actions) and pass them through untouched without affecting its cache entry, **as long as it doesn't introspect or invoke them**. This is how you keep a cached shell wrapped around dynamic content, or render a form whose action is dynamic, without losing the cache.
...
async function CachedShell({
  ...
  return (
    <div>
      ...
      <PublishButton action={publish} />{" "}
      {/* action passed through, never called here */}
    </div>
  );
}
```
