...
KIT_SOURCE="${VC_KIT_SOURCE:-https://github.com/withkynam/agent-skills-kit-pro-max-kit.git}"
VC_UPDATE_TMPDIR="/tmp/ag-update-$(date +%s)"
...
The manual snapshot-reading and diff logic in the previous version of this step is replaced by this invocation. The prose description of the algorithm is preserved in `references/ag-update.md` for reference.

**Fallback — no `.ag-installed-files` (first update with new system):**
When the snapshot file is absent, `compute-sync-plan.mjs` sets `priorSnapshot = []` (empty — no disk scan). No stale removal occurs via the snapshot path because there are no prior entries to compare against. `legacyDeletions` from the manifest are still applied independently (step 4 in the function) and may delete old paths that exist on disk. `.ag-installed-files` is written only when `--apply` is passed, not during a `--json` dry-run.
...
```
ag-update dry run: v{currentVersion} -> v{remoteVersion}

FILES:
  [modified]  .claude/agents/ag-execute-agent.md  (+12 -3)
  [new]       .claude/hooks/lib/new-util.cjs
  [removed]   .claude/skills/deprecated-skill/SKILL.md
  [unchanged] .claude/agents/ag-debugger.md
  ...
...
STALE WARNINGS: N paths failed the namespace guard (showing first 5 — see full compute-sync-plan output)
  .claude/skills/my-custom-ag-tool/SKILL.md
  ...

Summary: 5 modified, 2 new, 1 removal, 1 merge skipped, 45 unchanged
```

If `staleWarnings` is empty, omit the `STALE WARNINGS` section entirely. If non-empty, print the count and the first 5 entries only — do not dump all paths. Stale warnings indicate ag-namespace paths in your prior snapshot that are not in the kit's known `ownedPaths`. **If you have a custom ag-\* skill at one of those paths, rename it before applying the update** to prevent it from being deleted as a stale kit artifact.
...
**Large-delete WARNING:** After computing the dry-run summary, check `toDelete.length`. If it exceeds 20 files OR exceeds 10% of the prior install file count (line count of `.ag-installed-files`), print the following block prominently before asking for confirmation:
...
node "$VC_UPDATE_TMPDIR/compute-sync-plan.mjs" \
  ...
  --json | PROJECT_ROOT="$PROJECT_ROOT" node -e "
    ...
    const backupDir = path.join(root, '.agent-skills-kit-backup');
    ...
    if (fs.existsSync(backupDir) && fs.readdirSync(backupDir).length > 0) {
      const rotated = path.join(root, '.agent-skills-kit-backup-' + Math.floor(Date.now() / 1000));
...
(This preserves the pre-update versions of both overwritten and removed files, so any content is recoverable from `.agent-skills-kit-backup/`. If a `.agent-skills-kit-backup/` directory already exists from a prior run, it is rotated to a timestamped name (`.agent-skills-kit-backup-{unix-ts}/`) before the new backup is written, so earlier backups are never overwritten.)
...
- **Snapshot**: writes `.ag-installed-files` (sorted `managedFiles` — the `files` list from resolve-manifest; `legacyDeletions` are re-derived each run and not persisted).
- **Version**: writes `.ag-version` (manifest version string).
- **`.gitignore` guard**: after writing the version, apply ensures the project-root `.gitignore` contains `.agent-skills-kit-backup*/` (additive — creates the file if absent, appends the glob form `.agent-skills-kit-backup*/` if the glob form is missing even if a non-glob `.agent-skills-kit-backup/` line already exists, no-ops if the glob form is already present). This prevents rotated backup dirs (`.agent-skills-kit-backup-{ts}/`) from being accidentally committed.
- **staleWarnings (Type 1 — non-ag- namespace)**: paths in the prior snapshot that are not in the ag- namespace — moved to `toPreserve`, never deleted. Message form: `'X' in prior snapshot but not in kit namespace — preserved (verify manually)`.
- **staleWarnings (Type 2 — ag- namespace, `WARNING:` prefix)**: ag- namespace paths that are in `toDelete` but not in the known `ownedPaths`/`legacyDeletions` — these ARE deleted. The warning is advisory: if the user has a custom ag- skill at that path, rename it before updating.
...
```
ag-update complete: v{currentVersion} -> v{remoteVersion}
...
Snapshot written to .ag-installed-files
Version written to .ag-version: {remoteVersion}
```
...
Paste-ready hooks block: see https://github.com/withkynam/agent-skills-kit-pro-max-kit/blob/main/MIGRATION.md#action-required-settingsjson-hooks
or diff .claude/settings.json .agent-skills-kit-backup/.claude/settings.json
```
...
For each orphaned dir found, log a line to `.ag-orphaned-dirs.log` in the project root:
...
```
Found N orphaned dirs. See .ag-orphaned-dirs.log for details. Run ag-setup Merge Mode to migrate and cleanup.
```
...
If no orphaned dirs and no flat-plan signals are found, print nothing for Check C (no `.ag-orphaned-dirs.log` line written).
...
Recommended next step — run the five core validators:
  ...
  node .claude/skills/ag-audit-vc/scripts/validate-kit-portability.mjs
  node .claude/skills/ag-audit-context/scripts/validate-context-discovery.mjs
  node .claude/skills/ag-context-discovery/scripts/discover-skills.mjs
```
...
- Real working files outside `_seeds/` are preserved by default. The only allowed `process/` mutations inside ag-update are the safe old-layout migrations described in Step 8 / Step 10 Part D for `process/general-plans/` and `process/features/*/`.
...
- If `.ag-version` is missing, treat as version `0.0.0` (first update, apply everything).
...
- Removals are detected by comparing the local `.ag-installed-files` snapshot against the new resolved file list.
...
**Very old installs** (SKILL.md predating `legacyDeletions` support): use `install.sh` for a clean reinstall instead of `ag-update`.
