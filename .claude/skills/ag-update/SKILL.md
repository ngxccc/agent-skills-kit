...
Pull the latest agent harness improvements from the remote agent-skills-kit repository into the current project.
...
4. **Write snapshot**: Write the remote `files` list (sorted, one per line) to `.ag-installed-files`.

5. **Write version**: Write the manifest version string to `.ag-version`.
...
- Real working files outside `_seeds/` (`process/context/`, `process/features/`, `process/general-plans/`) are **NEVER** touched by ag-update.
