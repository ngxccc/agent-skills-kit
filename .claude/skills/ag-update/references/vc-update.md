...
Detailed reference for the ag-update skill.

## ag-manifest.json Schema Reference (v3.0.0)
...
```json
{
  ...
  "exclude": [
    ...
    ".claude/skills/ag-agent-browser/scripts/node_modules/**"
  ],
  "strip": [],
...
`CLAUDE.md` and `AGENTS.md` are harness-only files — overwritten freely on update like any other managed file. Project-specific content (context groups, tech stack, features) belongs in `process/context/all-context.md`, which ag-update never touches.

**Exception — safe legacy layout migration:** ag-update may migrate old-layout artifacts inside `process/general-plans/` and `process/features/*/` after the harness update completes, but only for safe cases where exactly one destination task folder can be inferred. Ambiguous or shared legacy artifacts are preserved and reported.
...
For each orphaned dir found, a line is logged to `.ag-orphaned-dirs.log` in the project root:
...
A stdout summary follows: `Found N orphaned dirs. See .ag-orphaned-dirs.log for details. Run ag-setup Merge Mode to migrate and cleanup.`
...
If `.ag-version` matches the remote manifest version, the deterministic file-sync is a no-op, but the adaptive legacy-layout migration (Part D) is NOT version-gated. Run the legacy-artifact scan first: if zero legacy artifacts exist, report "Already up to date — no legacy artifacts to migrate" and exit (no diff computed); if any legacy artifacts exist (e.g. `install.sh` just bumped `.ag-version` without running migration), report "Already up to date, but N legacy artifact(s) found — running content migration" and continue to the apply path so Part D migrates them (file diff stays empty; version unchanged).
...
```
ag-update dry run: v2.0.4 -> v2.1.0

FILES:
  [modified]  CLAUDE.md  (+15 -8)
  [modified]  .claude/agents/ag-execute-agent.md  (+3 -1)
...
```
ag-update complete: v2.0.4 -> v2.1.0
...
Version written to .ag-version: 2.1.0
