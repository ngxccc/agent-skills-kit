...
## ag-manifest.json Schema Reference (v2.1.0)
...
```json
{
  ...
  "exclude": [
    ...
    ".claude/skills/ag-chrome-devtools/scripts/node_modules/**"
  ],
  "strip": [],
...
| .ag-installed-files missing | Step 6 | Build synthetic snapshot from current filesystem, proceed |
...
If `.ag-version` matches the remote manifest version, report "Already up to date" and exit. No diff computed.
