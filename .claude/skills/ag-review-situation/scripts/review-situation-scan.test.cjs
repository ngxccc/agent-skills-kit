...
const {
  ...
  readPlan,
  renderText,
} = require("./review-situation-scan.cjs");
...
function createFixtureRepo() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "review-situation-scan-"));
  ...
  try {
    ...
    assert.ok(
      payload.worktrees.some(
        (worktree) =>
          fs.realpathSync.native(worktree.path) === expectedWorktree,
      ),
    );
    assert.ok(
...
test("task-folder convention plan is detected as primary plan", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "review-situation-task-folder-"),
  );
  const repo = path.join(root, "repo");
  fs.mkdirSync(repo);
  git(repo, ["init"]);
  ...
  // Task-folder convention: plan lives inside a named subfolder
  const taskFolderPlan = path.join(
    repo,
    "process",
    "general-plans",
    "active",
    "test_27-05-26",
    "test_PLAN_27-05-26.md",
  );
  writeFile(
    taskFolderPlan,
    ["# Test Task-Folder Plan", "", "**Status**: ACTIVE"].join("\n"),
  );
  git(repo, ["add", "."]);
  git(repo, ["commit", "-m", "feat: add task-folder plan"]);

  try {
    const payload = buildPayload(
      { fetch: false, planLimit: 10, maxPlanRefs: 20 },
      repo,
    );
    const allPlanPaths = [
    ...
    assert.ok(
      allPlanPaths.some(
        (p) =>
          p ===
          "process/general-plans/active/test_27-05-26/test_PLAN_27-05-26.md",
      ),
      `expected task-folder plan to be detected; found: ${JSON.stringify(allPlanPaths)}`,
    );
  } finally {
