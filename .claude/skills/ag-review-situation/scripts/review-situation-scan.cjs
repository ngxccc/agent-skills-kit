...
const PRIMARY_PLAN_PATTERNS = [
  /^[^/]+_PLAN_\d{2}-\d{2}-\d{2}\.md$/i,
  /^PLAN\.md$/i,
  /^plan\.md$/i,
];
...
Usage:
  node .claude/skills/ag-review-situation/scripts/review-situation-scan.cjs [--json] [--fetch]
...
function getCurrentState(root, worktrees, warnings) {
  ...
  const currentWorktree =
    worktrees.find(
      (record) => path.resolve(record.path) === path.resolve(root),
    ) || null;
...
function rankRefs(refs, state, worktrees) {
  const checkedOut = new Set(
    worktrees.map((record) => record.branch).filter(Boolean),
  );
  return refs
    .map((ref) => {
      let rank = 0;
    ...
    .sort(
      (a, b) =>
        b.rank - a.rank || b.time - a.time || a.name.localeCompare(b.name),
    );
}
...
function hasIncompletePhase(content) {
  ...
  for (const line of content.split("\n")) {
    ...
    if (candidates.some((cell) => cell && isIncompleteStatusCell(cell)))
      return true;
  }
...
function getFilesystemActivePlanFiles(root) {
  ...
  return roots
    ...
    .filter((file) => isActivePlanPath(path.relative(root, file)));
}
...
function rankPlan(plan, current) {
  ...
  for (const source of plan.sources || [plan.source]) {
    if (
      source.worktree &&
      path.resolve(source.worktree) === path.resolve(current.root)
    )
      score += 1000;
    if (
      current.branch &&
...
function sortPlans(plans, current) {
  return plans.sort(
    (a, b) =>
      rankPlan(b, current) - rankPlan(a, current) ||
...
function buildWarnings(payload) {
  ...
  if (payload.selectedPlanHint?.advisory) {
    warnings.push(
      `Selected plan is advisory (${payload.selectedPlanHint.source}) and does not replace explicit execute approval.`,
    );
  }
  ...
  if (payload.refs.total > payload.options.maxPlanRefs) {
    warnings.push(
      `Tracked plan scan limited to ${payload.options.maxPlanRefs} ranked refs out of ${payload.refs.total}.`,
    );
  }
...
function buildNextSteps(payload) {
  ...
  if (payload.selectedPlanHint) {
    steps.push(
      `If execution should continue, confirm the selected plan explicitly: ${payload.selectedPlanHint.path}.`,
    );
  ...
  if (blockedPlan)
    steps.push(
      `Check blocker status for ${blockedPlan.path} before resuming work.`,
    );
  ...
  if (remoteBranch)
    steps.push(
      `Review recent remote branch ${remoteBranch.name} before duplicating effort.`,
    );
...
function renderText(payload) {
  ...
  if (payload.current.ahead || payload.current.behind) {
    lines.push(
      `- Ahead/behind: +${payload.current.ahead} / -${payload.current.behind}`,
    );
  }
  if (payload.selectedPlanHint) {
    lines.push(
...
function buildPayload(rawOptions, cwd = process.cwd()) {
  ...
  const filesystemPlans = sortPlans(
    dedupePlans(scanFilesystemPlans(worktrees, warnings)),
    current,
  );
  const trackedPlanRefs = refs.slice(0, options.maxPlanRefs);
  const trackedPlans = sortPlans(
    dedupePlans(scanTrackedPlans(root, trackedPlanRefs, warnings)),
    current,
  );
  const mergedPlans = sortPlans(
    dedupePlans([...filesystemPlans, ...trackedPlans]),
    current,
  );
  const unfinished = mergedPlans
    .filter((plan) => plan.unfinished)
    .slice(0, options.planLimit);
  const selectedPlanHint = resolveSelectedPlanHint(
    root,
    options,
    filesystemPlans,
  );
