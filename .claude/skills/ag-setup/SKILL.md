...
# Agent Skills Agent Harness Setup
...
The `install.sh` script handles fetching and installing harness files before ag-setup runs. For existing projects, it backs up old `.claude/`, `.codex/`, `.agents/` to `.agent-skills-backup/`, then does a clean install of all kit files. User's `.claude/settings.json` is restored after install. The `process/` directory is never touched by install.sh -- layout migration happens in ag-setup's SCAFFOLD phase.
