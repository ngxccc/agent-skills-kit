...
ag-setup version 3.3.0 — (independent of kit version; see SKILL.md for documentation discipline)

> **CRITICAL — Dual-File Synchronization:** This document and `.claude/skills/ag-setup/SKILL.md` must be edited together. Any change to Merge Mode logic, safe-inference rules, or migration flow MUST appear in both files in the same commit. Dual-file drift creates inconsistent user behavior.

Detailed instructions for each phase of the ag-setup skill, including interactive flows for new and existing projects.
...
| `process/` exists but contains ONLY kit-installed files (`_seeds/`, `development-protocols/`, `context/generated-skills-catalog.json`) and no user content (`all-context.md` absent, `general-plans/` absent, `features/` absent or empty) | **New project** -- use Flow A (install.sh ran but ag-setup has not yet run; this is a fresh install)           |
...
| Has `.agent-skills-kit-backup/` (just ran install.sh over an existing setup)                                                                                                                                                               | **Existing project** -- use Flow B                                                                             |
...
- `.agent-skills-kit-backup/` (indicates install.sh just ran over an existing setup)
...
6. Retain `.seed` originals: copy the original `.seed` files alongside the populated real files in the target `process/` directory. These serve as structural reference companions -- agents and future `ag-update` runs can diff populated files against their `.seed` originals to detect structural drift or missing sections
...
| Example PRDs at old locations (under `process/context/`, `process/context/planning/`, or `process/development-protocols/references/`)                         | Move to `.claude/skills/ag-generate-plan/references/`.                                                                                                                                                                                                                                       |
...
| `process/general-plans/reports/`, `process/general-plans/references/`, or `process/features/*/reports/`, `process/features/*/references/` sibling directories | Auto-migrate every **safe** legacy artifact into the relevant task folder in the same scope. Delete the legacy dir if it becomes empty. Leave only unresolved artifacts behind, list them in LAYOUT CHANGES, and do NOT create new sibling `reports/` or `references/` dirs during scaffold. |
...
- **Safe legacy artifact migration contract:** for sibling `reports/` / `references/` dirs, a move is automatic only when exactly one destination task folder can be inferred in the same scope. Safe inference signals are:
  - the artifact basename starts with one task slug and only one matching `{slug}_{date}/` folder exists
  - the artifact basename or path contains an exact `{slug}_{date}` token matching one task folder
  - the scope has exactly one task folder total
  - exactly one task-folder plan references that legacy artifact path or basename
- Leave an artifact in place when multiple task folders match, multiple plans reference it, the file is clearly shared across tasks, or no task folder exists yet in that scope.
- Preserve the original filename when moving unless there is a collision; in that case append `-migrated`.
- The intended end-state after successful safe migration is that each feature folder and `process/general-plans/` contain only `active/`, `completed/`, and `backlog/`. A remaining legacy `reports/` or `references/` dir means unresolved artifacts still need manual review.

#### Migration Flow (orphaned-dir detection → display → confirm → execute)

This mirrors the Merge Mode auto-migrate flow in `SKILL.md` — same rationale, same rules, same fallback logic. The two docs MUST stay consistent (dual-file synchronization).

1. **Detect** orphaned dirs: scan for `process/reports/`, `process/references/`, and feature-level `process/features/*/reports/`, `process/features/*/references/` sibling dirs (plus pre-v3 `process/plans/`, `process/skills/` leftovers).
2. **Display** a migration summary table — each row: source path, inferred destination task folder, file count, risk level (safe vs needs-review). Surface under the LAYOUT CHANGES section of PRESENT & ASK.
3. **Confirm** — one-button confirmation: "Migrate and clean up all stale artifacts". The user approves before any move executes. Never move files silently.
...
- **NOT safe** — no matching plan slug found, ambiguous filenames, multiple task folders match, or conflict with an existing dir → do NOT create any destination folder; show the fallback message below and pause for manual review.

##### Fallback Display Message

When orphaned dirs are detected but no plan slug can be inferred from project files (no safe inference possible), ag-setup displays:
...
The STUDY phase is the core value of ag-setup v3. It transforms scaffolded seed files into ready-to-use context by actively scanning the codebase.
...
The STUDY phase is the most resource-intensive part of ag-setup. The executing agent SHOULD spawn parallel subagents to maximize throughput and avoid context window exhaustion.
...
| G: ag-generate-context (delegation) | Findings from C (approved-groups list) | DELEGATION: invoke `ag-generate-context` skill in `setup-delegation` mode, passing the approved-groups list from Subagent C. Context-group detection and per-group file authoring is owned by `ag-generate-context` — see `.claude/skills/ag-generate-context/references/generate-context.md` for the detection table and per-mode instructions. |
...
Context Group Detection Table: see `.claude/skills/ag-generate-context/references/generate-context.md` section "Context Group Detection Table" for the canonical detection signals, group names, and creation rules. That file is the single source of truth for group detection — do not duplicate the table here.
...
```bash
ls -la .agents/skills
ls .agents/skills/ag-setup/SKILL.md
```
...
3. Run the ag-audit-context skill to validate context discovery wiring
