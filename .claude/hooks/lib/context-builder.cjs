...
function resolveRulesPath(filename, configDirName = '.claude') {
  ...
  for (const fallbackFilename of fallbackFilenames) {
    ...
    if (fs.existsSync(localRulesPath)) return `${configDirName}/rules/${fallbackFilename}`;
    if (fs.existsSync(globalRulesPath)) return `~/.claude/rules/${fallbackFilename}`;
  }

  // Backward compat: try workflows/ (legacy location)
  for (const fallbackFilename of fallbackFilenames) {
    const localWorkflowsPath = path.join(process.cwd(), configDirName, 'workflows', fallbackFilename);
    const globalWorkflowsPath = path.join(os.homedir(), '.claude', 'workflows', fallbackFilename);
    if (fs.existsSync(localWorkflowsPath)) return `${configDirName}/workflows/${fallbackFilename}`;
    if (fs.existsSync(globalWorkflowsPath)) return `~/.claude/workflows/${fallbackFilename}`;
  }
...
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.claude', baseDir } = {}) {
  ...
  // Resolve paths
  const devRulesPath = resolveRulesPath('implementation-standards.md', configDirName);
