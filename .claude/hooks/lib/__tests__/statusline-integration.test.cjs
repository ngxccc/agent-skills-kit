...
try {
  ...
  test("Git input processed without error", () => {
    ...
    console.log(
      `  Skipped (expected in some environments): ${e.message.substring(0, 60)}`,
    );
  });
} finally {
...
try {
  ...
  test("Output includes percentage indicator", () => {
    ...
    assertTrue(
      /\d+%|█|░/.test(contextResult),
      "Should include context bar or percentage",
    );
  });
...
try {
  const invalidResult = execSync(
    `echo 'not valid json'  | node .claude/statusline.cjs`,
    {
      encoding: "utf8",
      stdio: ["pipe", "pipe", "pipe"],
    },
  ...
  test("Invalid JSON produces fallback output", () => {
    ...
    assertContains(
      invalidResult,
      "📁",
      "Fallback should include directory emoji",
    );
  });
...
try {
  ...
  console.log(
    `  Sample: ${multilineResult.trim().split("\n")[0].substring(0, 80)}...`,
  );
} catch (e) {
  test("Multi-line output generates content", () => {
    throw e;
...
try {
  ...
  test("NO_COLOR=1 produces output", () => {
    assertTrue(
      noColorResult.length > 0,
      "Should produce output with NO_COLOR=1",
    );
  });
...
try {
  ...
  test("Narrow terminal wraps to multiple lines", () => {
    assertTrue(
      narrowLines >= 2,
      `Should wrap to multiple lines, got ${narrowLines}`,
    );
  });
  ...
  if (wideLines > 0) {
    test("Narrow has more lines than wide", () => {
      assertTrue(
        ...
        "Narrow should have equal or more lines",
      );
    });
  }
} catch (e) {
...
const longPathInput = JSON.stringify({
  ...
  workspace: {
    current_dir:
      "/home/user/very/deeply/nested/directory/structure/project/source",
  },
  context_window: { context_window_size: 200000 },
});
...
try {
  ...
  console.log(
    `  Output: ${longPathResult.trim().split("\n")[0].substring(0, 80)}...`,
  );
} catch (e) {
  test("Long path produces output without crash", () => {
    throw e;
...
const tmpSessionPath = path.join(os.tmpdir(), `ag-session-${sessionId}.json`);
fs.writeFileSync(
  tmpSessionPath,
  JSON.stringify(
    {
...
try {
  ...
  test("Agent tracking shows completed agent", () => {
    ...
    const hasAgent =
      agentTodoResult.includes("research-agent") ||
