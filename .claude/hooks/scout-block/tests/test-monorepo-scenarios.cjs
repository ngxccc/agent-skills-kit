...
const scenarios = [
  ...
  },
  {
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'cd apps/api/node_modules' },
    },
    expected: 'BLOCKED',
  ...
  },
  {
    input: {
      tool_name: 'Bash',
      tool_input: {
        command: 'cat packages/shared/node_modules/lodash/index.js',
      },
    },
    expected: 'BLOCKED',
  ...
  },
  {
    input: {
      tool_name: 'Read',
      tool_input: { file_path: 'packages/web/node_modules/react/package.json' },
    },
    expected: 'BLOCKED',
  ...
  },
  {
    input: {
      tool_name: 'Grep',
      tool_input: { pattern: 'export', path: 'packages/web/node_modules' },
    },
    expected: 'BLOCKED',
  ...
  },
  {
    input: {
      tool_name: 'Glob',
      tool_input: { pattern: 'packages/web/node_modules/**/*.js' },
    },
    expected: 'BLOCKED',
  ...
  // === Deep nesting (also bug cases) ===
  {
    input: {
      tool_name: 'Read',
      tool_input: { file_path: 'a/b/c/d/node_modules/pkg/index.js' },
    },
    expected: 'BLOCKED',
  ...
  },
  {
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'ls packages/web/dist' },
    },
    expected: 'BLOCKED',
  ...
  },
  {
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'cat apps/api/build/server.js' },
    },
    expected: 'BLOCKED',
  ...
  },
  {
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'pnpm --filter web run build' },
    },
    expected: 'ALLOWED',
  ...
  },
  {
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'ls packages/web/src' },
    },
    expected: 'ALLOWED',
    desc: 'ls safe path',
  },
  {
    input: {
      tool_name: 'Grep',
      tool_input: { pattern: 'import', path: 'src' },
    },
    expected: 'ALLOWED',
    desc: 'Grep in src',
  },
  {
    ...
    expected: 'BLOCKED',
    desc: 'Glob all .ts files at root',
  },
  {
    input: {
      tool_name: 'Bash',
      tool_input: { command: 'find packages -name "*.json" | head' },
    },
    expected: 'ALLOWED',
  ...
  // === Edge cases - names containing blocked words but NOT the dirs ===
  {
    input: {
      tool_name: 'Read',
      tool_input: { file_path: 'my-node_modules-project/file.js' },
    },
    expected: 'ALLOWED',
...
for (const scenario of scenarios) {
  try {
    ...
    if (success) {
      ...
      console.log(
        `\x1b[31m✗\x1b[0m ${scenario.desc}: expected ${scenario.expected}, got ${actual}`,
      );
      failed++;
    }
  } catch (error) {
    ...
    if (success) {
      ...
      if (error.stderr) {
        console.log(
          `  stderr: ${error.stderr.toString().trim().split('\n')[0]}`,
        );
      }
      failed++;
    }
