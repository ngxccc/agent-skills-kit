...
- **Workflow Documentation Standard:** Architectural and feature workflow specifications must follow the SSOT standard at `process/development-protocols/references/workflow-documentation-standard.md` (via `ag-docs` skill design mode). Documents use `docType: feature-workflow` or `docType: infrastructure-workflow`, include a 4-level WBS table, autonumbered Mermaid sequence diagrams, and are saved using kebab-case naming to `docs/design/<feature-topic>-workflow.md`. Grounding via Codebase Memory MCP graph tools is mandatory.
