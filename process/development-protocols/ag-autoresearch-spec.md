---
name: protocol:ag-autoresearch-spec
description: "Deeper design reference for the ag-autoresearch loop primitive (PVL/EVL bookkeeper); SKILL.md is the operative contract."
date: 09-06-26
metadata:
  node_type: memory
  type: protocol
...
> **STATUS: ACTIVE**
>
> ag-autoresearch is the live loop primitive for the RIPER-5 system. It is the shared
...
>
> **Authority:** `.claude/skills/ag-autoresearch/SKILL.md` is the operative contract; this
> document is the deeper design reference. Where the two diverge, the SKILL.md wins.

# ag-autoresearch — Specification
...
All three loops in the ag-system follow this shape:

| | What we ran (I1–I11) | PVL | EVL |
|---|---|---|---|
| **Corpus** | spec .md files + agent files | plan .md file | changed source + test files |
| **Research step** | 2 parallel agents scan for behavioral gaps | ag-validate-agent runs V1-V7 gates | ag-tester runs validate-contract gate commands |
| **Gap signal** | agents list findings with severity | FAIL / CONDITIONAL / CONCERN | failing gate command (non-zero exit) |
| **Fix step** | 3 parallel agents edit the files | ag-plan-agent supplements the plan | ag-execute-agent fixes failing code (supplement mode) |
...
| **Commands** | configured per invocation | V1-V7 gate sequence (ag-validate-agent owns) | exact commands from validate-contract "Test gates" field — never invented |
...
The domain-specific details (which agents run, what the gates check, how supplementing works) stay where they are. ag-autoresearch provides the loop mechanics: iteration counter, convergence detection, iteration reports, plateau detection.
...
When ag-autoresearch runs a plan-validate-fix or execute-validate-fix loop in an autopilot session:
...
- PVL: V1-V7 gate sequence, SUPPLEMENT REQUEST format, validate-contract write, known-gap exclusion, supersedes: logic → stays in ag-validate-agent and ag-plan-agent
- EVL: which gate commands to run, execute-supplement scoping, HANDOFF SUMMARY format, agent-probe re-invocation → stays in ag-tester and ag-execute-agent
...
ag-autoresearch is invoked directly — no RIPER-5 plan or validate-contract required.
...
- **Safety check:** `node .claude/skills/ag-audit-context/scripts/validate-context-discovery.mjs`
...
- Fewer subcommands — only the ones not already covered by existing ag-system skills

Already covered by existing skills (don't port): debug → ag-debugger, security → ag-security, scenario → ag-scenario, predict → ag-predict.
...
**Hook conflict note:** autoresearch hooks are installed at `.claude/hooks/autoresearch/`. Do NOT register scout-block or privacy-block from that set — the ag-system already has superior versions registered in settings.json. Only iteration-context.cjs and stop-notify.cjs are safe to keep.

---

## Implementation backlog

- [ ] Write SKILL.md at `.claude/skills/ag-autoresearch/SKILL.md`
- [ ] Remove `ag-autoresearch` from the Removed Skills table in ag-system-behavior-reference.md (prerequisite before any implementation)
- [ ] Add `ALL_CLEAR` and `REGRESSION_DETECTED` to signal vocabulary in ag-system-behavior-reference.md
- [ ] Add entry to CLAUDE.md skill registry (under contract skills)
- [ ] Annotate PVL and EVL sections in ag-system-behavior-reference.md with "ag-autoresearch(domain=plan)" and "ag-autoresearch(domain=execution)" for clarity
- [ ] Adapt `iteration-context.cjs` hook from autoresearch repo: scan `process/features/*/active/*/results.tsv` (not project-root `autoresearch/` paths); additive injection only, do not duplicate ag-system fields
- [ ] Adapt `session-init.cjs`: path change `plans/` → `process/general-plans/active/` and `process/features/*/active/`
- [ ] Skip `scout-block.cjs` and `privacy-block.cjs` — ag-system settings.json has superior versions; registering these would create CRITICAL conflicts
...
**D1 — Git commits during loop:** No per-iteration git commits. The autoresearch repo commits each iteration to enable `git revert` rollbacks; ag-autoresearch uses iteration reports + TSV instead. Regression detection (5 in termination conditions above) is the rollback signal — a REGRESSION halt lets the user inspect and revert manually. Single conventional commit via ag-git-manager at loop end (or not at all if the session continues to further phases). Rationale: keeps git history clean, avoids "WIP iteration N" noise in log.
