---
name: protocol:ag-system-behavior-04-research
...
**Agent:** `ag-research-agent` (sonnet)
...
### [R-S1] ag-context-discovery (Tier 0) — REQUIRED
...
### [R-S2] ag-plan-discovery (Tier 0) — REQUIRED
...
### [R-S3] ag-review-situation (Tier 0) — REQUIRED

Check branch, worktree, and active plan state. Understand what is currently in flight.

### [R-S4] ag-agent-strategy-compare (Tier 0) — REQUIRED

Run this before spawning any research subagents. Score the full 4-option strategy suite (sequential / parallel / workflow / ag-team) with cost estimates. Do **NOT** pause separately — the options are surfaced for confirmation inside the Combined Clarification Gate, alongside the intent questions (single trip).
...
### [R2] ag-docs-seeker — (Tier 1) — REQUIRED on first library/framework encounter

When you first encounter a library, framework, or SDK: run ag-docs-seeker before relying on your training data. Pass: library name, version from package.json, and your specific question. Include the output as evidence in your findings.

### [R3] ag-sequential-thinking — (Tier 2) — CONDITIONAL
...
### [R4] ag-agent-strategy-compare — (Tier 2) — CONDITIONAL (mid-phase)
...
### [R5] ag-test-coverage-plan — (Tier 1) — REQUIRED before finalizing findings
...
### [R6] ag-validate-findings — (Tier 2) — OPTIONAL
...
### [R7] ag-problem-solving — (Tier 2) — CONDITIONAL
...
- any optional deep work offered as a _choice_, not a separate pause (e.g. run `ag-validate-findings`)
- if 2+ distinct investigation directions were found: a fan-out recommendation

This is the single end-of-phase ask. Do not split strategy confirmation and "go" into separate round-trips. Under `/goal` the gate auto-proceeds on the recommended option — including a re-RESEARCH loop, bounded by the active-loop cap (`ag-autoresearch`, 10-cycle ceiling) to prevent infinite re-research. See `12-reference.md` (`PHASE-GATES`).
