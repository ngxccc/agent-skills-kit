---
name: protocol:ag-system-behavior-07-plan
...
**[P-S1] ag-context-discovery** (Tier 0) — REQUIRED (Part A + Part B frontmatter)

Load relevant context files. Required.

**[P-S2] ag-plan-discovery** (Tier 0) — REQUIRED (alongside P-S1, Part A + Part B frontmatter)

Run alongside P-S1. Find related plans to avoid duplication.

**[P-S3] ag-review-situation** (Tier 0) — REQUIRED

Confirm the active plan and branch. Understand what was done before.

**[P-S4] ag-agent-strategy-compare** (Tier 0) — REQUIRED (strategy for this phase)
...
**[P1] ag-scout** (Tier 1) — REQUIRED (step 0 — before reading any existing plans)
...
> **Pattern divergence detection criteria:** A pattern is established if 2 or more files in the same package or blast-radius use the same structural approach. Divergence threshold: the planned approach uses a different structural approach than 2+ existing files in the same domain. ag-scout output must be explicitly compared to the planned approach before concluding divergence. If there are 0 relevant files, no divergence note is needed.

**[P2] ag-generate-plan** (Tier 1) — REQUIRED

All plan creates and updates go through this skill's schema.

**[P3] ag-test-coverage-plan** (Tier 1) — REQUIRED DURING drafting (TDD-first, not after). Full test-context-discovery + exhaustive 3-strategy scenario enumeration is required by design.
...
**[P4] ag-generate-phase-program** (Tier 1, conditional) — REQUIRED when shape = PHASE PROGRAM
...
- `ag-generate-phase-program/templates/umbrella-plan-template.md`
- `ag-generate-phase-program/templates/phase-stub-template.md`
- `ag-generate-phase-program/references/program-goal-charter-template.md`

**[P5] ag-agent-strategy-compare** (Tier 0) — REQUIRED when phase program shape detected

Run BEFORE creating any files. This picks the execution strategy for the program (how the N phase plans will be created).

> **Second distinct call:** This is a second `ag-agent-strategy-compare` call, distinct from Tier-0 P-S4. P-S4 sets strategy for THIS PLAN phase's drafting. P5 sets strategy for PHASE PROGRAM execution (how the N phase plans will be created — agent-team vs parallel-subagents). Pass explicitly distinct context.

If phases have dependencies: recommend sequential-first ordering.

**[P6] ag-predict** (Tier 1) — REQUIRED for COMPLEX plans

Run before writing the Implementation Checklist. Five-persona debate. Output goes into the plan's Risk Predictions section.

Note: INNOVATE's ag-predict debates which approach to pick. PLAN's ag-predict debates how to implement the chosen approach. Pass the chosen approach and planned implementation details, not the approach alternatives. When INNOVATE was skipped (mechanical "how"), PLAN's ag-predict debates the implementation of the SPEC's single obvious path — there is no chosen-approach handoff, so pass the SPEC's behavioral outcomes and the planned implementation.

**[P7] ag-scenario** (Tier 2) — REQUIRED for 2-3 highest-risk checklist items

Add edge cases into the checklist and test gates.

**[P8] ag-security** (Tier 2) — REQUIRED if auth/billing/secrets surface touched

Run before writing the Public Contracts and Blast Radius sections.

**[P9] ag-sequential-thinking** (Tier 1) — REQUIRED if 3+ interdependent phases

Verify ordering before writing the phase sequence.

**[P10] ag-docs-seeker** (Tier 1) — REQUIRED (mandatory, not conditional)

Run before any checklist step that references a library API signature.

**[P11] ag-validate-findings** (Tier 2) — OPTIONAL (recommended for COMPLEX plans)

Self-check before handoff. Catches gaps before ag-validate-agent runs the full suite.
...
2. **Recommended next step (marked recommended), bidirectional:**
   ...
   - **Re-run PLAN (loop back)** — when `validate-plan-artifact.mjs` fails, the test-tier matrix has gaps, or `## Test Infra Improvement Notes` is incomplete. Name the specific gaps + questions feeding the next entry gate. Bounded by the ag-autoresearch 10-cycle cap.
3. **Recommended strategy** for PVL — full 4-option suite with 7-signal score + cost, one marked recommended, as selectable choices.
4. **Optional deep work** (P11 ag-validate-findings self-check) offered as a _choice_, not a pause.
...
- ag-agent-strategy-compare was run for PVL and surfaced inside the Phase-End Recommendation Gate (not a separate pause)
