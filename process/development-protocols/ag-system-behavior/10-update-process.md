---
name: protocol:ag-system-behavior-10-update-process
...
**[U-S0] ag-intent-clarify** — REQUIRED FIRST. Restate the scope of what is being updated and archived. Do **NOT** add a separate confirmation pause — the restatement rides on the `ENTER UPDATE PROCESS MODE` entry trigger as the single entry gate.

**[U-S1] ag-context-discovery** — REQUIRED. Load context for this update pass (Part A + B frontmatter).

**[U-S2] ag-plan-discovery** — REQUIRED. Run alongside U-S1.

**[U-S3] ag-review-situation** — REQUIRED. Check branch, worktrees, and in-flight work before making archival decisions.

**[U-S4] ag-agent-strategy-compare** — REQUIRED. Decide the strategy for this update-process pass.
...
**[U-S5] ag-generate-closeout** — REQUIRED. Run this before archiving any plan or updating umbrella state. It produces the 9-field closeout packet.

---

## Required Skills — Phase 3 (run after auditing what changed)

**[U1] ag-audit-vc** — REQUIRED if any agent, skill, or `.claude/` file was modified.
...
**[U2] ag-audit-context** — REQUIRED if any `process/context/` file or context group was modified. Also REQUIRED if the EVL handoff summary contains any `CONTEXT_PARTIAL: [area]` flags — even if no context file was directly modified.
...
**[U3] ag-audit-plans** — REQUIRED at natural stopping points (feature complete or "what's next?").
...
```bash
node .claude/skills/ag-audit-plans/scripts/validate-umbrella-state.mjs
node .claude/skills/ag-audit-plans/scripts/validate-phase-reports.mjs
node .claude/skills/ag-audit-plans/scripts/validate-backlog-notes.mjs
```
...
The `ag-generate-closeout` skill produces this packet. All 9 fields are required. Emit it as a fenced block in the session chat before any archival action. Also write it to the phase report under `## Closeout Packet`.
...
4. **What was verified vs still unverified** — test evidence vs pending verification
   ...
   - **Recovery path when absent:** If the validate-contract is absent and no skip reason was documented, classify the plan as "Keep in active/ — needs further testing" — not "Ready for archival". Write a backlog note: "missing validate-contract — plan needs PVL before archival." Do not route back to ag-validate-agent. This is a classification constraint, not a blocker.
...
7. **Commit-checkpoint recommendation** — invoke ag-git-manager now or after UPDATE PROCESS?
...
`ag-generate-closeout` fires at two points:
...
**Under /goal:** Drift scoring is informational only. It populates the phase report but does not produce a recommendation phrase (the agent is already in UPDATE PROCESS). If drift is 3 or higher, automatically create follow-up plan stubs for gap items. Do not spawn ag-execute-agent autonomously without PVL validation — that would bypass inner loop gates. Archive the current plan; the follow-up stub is the forward path.

### Drift Escalation

- Drift 1–2 → backlog entry.
- Drift 3+ → create a follow-up plan stub for the gap items. Under /goal: satisfy via auto-stub creation (do NOT spawn ag-execute-agent without PVL); archival proceeds — never a live block.
...
1. **Update the plan file.** Mark completed checklist items. Record any deviations from the plan. Tick the Step 7 checkbox in `## Phase Loop Progress`: `- [x] 7. UPDATE PROCESS — archived; context updated; committed`. Do not modify the `## Validate Contract` section — that is owned by ag-validate-agent. If a validate-contract update is needed (e.g., correcting the `supersedes:` field): spawn ag-validate-agent in contract-update mode.
...
2. **Recommended next step (marked recommended), bidirectional** — driven by drift:
   ...
   - **Start a new phase program** (recommended when natural next work was identified) — route through `ag-generate-phase-program` to scaffold the umbrella + stubs.
   - **Loop back on drift** (recommended when drift is HIGH 3+ and gap items remain) — create follow-up plan stubs / re-open targeted work. Bounded by the ag-autoresearch 10-cycle cap. Under /goal: auto-create stubs (per Drift Signal Scoring); do NOT spawn ag-execute-agent without PVL.
...
- [U-S1] ag-context-discovery ran
- [U-S2] ag-plan-discovery ran
