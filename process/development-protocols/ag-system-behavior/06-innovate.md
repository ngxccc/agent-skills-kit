---
name: protocol:ag-system-behavior-06-innovate
...
**Agent:** `ag-innovate-agent` (sonnet)
...
### [I-S0] ag-intent-clarify (Tier 0) — REQUIRED
...
### [I2] ag-docs-seeker (Tier 1) — REQUIRED on any library-dependent approach
...
### [I2.5] ag-feasibility-test — Conditional (runs BEFORE [I3])

Use this when an approach candidate hinges on an unverified external/runtime mechanism. Emit `VC-FEASIBILITY-PROBE-NEEDED: [hypothesis]` and halt. Do NOT proceed to ag-predict until the verdict returns. See `orchestration.md §VC-FEASIBILITY-PROBE-NEEDED Signal Routing`. Note: VALIDATE (Layer 2) is also a co-emitter of this signal; see `08-validate.md [V2-PROBE]`.

### [I3] ag-predict (Tier 1) — REQUIRED before writing the Decision Summary
...
**Deep mode:** ag-predict runs in SIMPLE mode by default. If the approach touches a pattern that has failed before in this codebase, or a known risky surface: the INNOVATE agent emits exactly this signal and stops:

```
VC-PREDICT-DEEP-NEEDED: [surface/pattern] — pausing for research subagent.
```

The orchestrator then spawns a research subagent to gather deeper context, then re-spawns the INNOVATE agent with the findings. The INNOVATE agent does not move forward until ag-predict completes. This is an orchestrator-driven re-spawn, **not** a user pause — it does not count against the two-gate single-trip rule; user-facing optional deep work is offered at the Phase-End Recommendation Gate, not mid-phase.

### [I4] ag-scenario (Tier 2) — CONDITIONAL
...
### [I5] ag-security (Tier 2) — CONDITIONAL
...
### [I6] ag-sequential-thinking (Tier 2) — CONDITIONAL
...
### [I7] ag-agent-strategy-compare (Tier 2) — CONDITIONAL (mid-phase)
...
### [I8] ag-problem-solving (Tier 2) — CONDITIONAL
...
- ag-predict MUST be invoked as a real skill call, not summarized or mentioned.
...
2. **Recommended next step (marked recommended), bidirectional:**
   ...
   - **Re-run INNOVATE (loop back)** — when fewer than two viable approaches survive, ag-predict surfaced an unmitigated high-risk threat, or accepted constraints conflict. Name the specific gaps + questions that feed the next entry gate. Bounded by the ag-autoresearch 10-cycle cap.
...
4. **Optional deep work** (ag-scenario, ag-security, ag-sequential-thinking, parallel approach exploration from I7) offered as _choices_, not a pause.
...
[ag-predict 5-persona output, verbatim or summarized per persona]
...
- ag-predict was invoked; the debate is included in the Decision Summary under "Risk Predictions"
...
- ag-agent-strategy-compare was run for PLAN and surfaced inside the Phase-End Recommendation Gate (not a separate pause)
