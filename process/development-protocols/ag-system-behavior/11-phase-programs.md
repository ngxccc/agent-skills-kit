---
name: protocol:ag-system-behavior-11-phase-programs
...
The outer PLAN step produces two artifacts via `ag-generate-phase-program`:
...
**When to use agent team:** `ag-agent-strategy-compare` must recommend **agent-team** — not parallel-subagents — when 3 or more phase plans are being created. Agent team members communicate. Parallel subagents cannot.
...
After all phase plan agents complete, the orchestrator reads all `## Potential Blast Radius Conflicts` sections. For each flagged conflict, resolve it and write a `## Pre-PVL Conflict Resolution` section to the umbrella plan before spawning ag-validate-agent.
...
**Completion check:** Before spawning ag-validate-agent, confirm that all `Action: update Phase [X] blast-radius claim` entries have actually been applied. If not: the conflict is unresolved.
...
1. Spawn a single ag-plan-agent for the conflicted phase only (not a full agent-team re-run).
...
STEP 0: Tier-0 Skills (MANDATORY at entry of EVERY step)
  ag-intent-clarify
  ag-context-discovery (Part A + B frontmatter)
  ag-plan-discovery (alongside context-discovery)
  ag-review-situation
  ag-agent-strategy-compare (strategy for THIS step's specific work)
```
...
**Phase-level loop-back.** A phase's exit Phase-End Recommendation Gate is bidirectional: it may recommend advancing to the next phase OR re-running the same phase (e.g., inner PVL CONDITIONAL with unresolved gaps, or EVL classification "Keep in active/ — needs further testing"). Phase-level loop-backs are bounded by the ag-autoresearch 10-cycle cap (the same cap that bounds inner PVL and EVL). Under /goal these loop-backs are autonomous; they do not add a user gate.
...
STEP 1: RESEARCH (full — scoped to this phase)
  ...
  Run ag-test-coverage-plan (Part A + B + C + D).
  Emit infra improvement suggestions for any agent-probe/known-gap tiers found.
```

```
STEP 2: INNOVATE (full — scoped to this phase)
  Explore 2+ approaches. Run ag-predict. Produce Decision Summary.
  Do not skip ag-predict because "the outer loop already decided the approach."
...
STEP 3: PLAN-SUPPLEMENT (update the existing phase plan with inner R+I findings)
  ag-plan-agent in supplement mode — update the existing plan in place.
...
Under /goal: when Step 3 completes, ag-plan-agent emits:
...
ag-validate-agent emits: `PHASE_SKIPPED: BLOCKED — [phase N] backlog note written; advancing to Phase [N+1]`
...
STEP 5: EXECUTE (fully autonomous)
  ...
  ag-execute-agent also checks the program registry at Step 0. If both the current phase AND its immediate prerequisite phase are BLOCKED-skipped: ag-execute-agent emits `CASCADE_BLOCKED:` and suspends.
...
STEP 7: UPDATE PROCESS for this phase
  ...
  Run validators: ag-audit-vc, ag-audit-context, ag-audit-plans as applicable.
...
1. Run it as an EVL-style loop, bounded by the same ag-autoresearch 10-cycle cap that bounds inner PVL and EVL.
...
- CONTEXT_PARTIAL warning from ag-context-discovery (proceed with best-effort context; note partial context in phase report)
...
- **Autonomous plan creation PVL:** Autonomous creation includes: writing the plan file + registering it in the umbrella + triggering inner PVL (via ag-validate-agent) before EXECUTE begins. PVL for new mid-program plans follows phase program V6 semantics: write validate-contract only; do NOT write a new /goal block; the umbrella's Stable Program Goal remains authoritative.

  **Skill sequence for mid-program plan creation:** Use abbreviated PLAN scope: Tier-0 skills + ag-scout + ag-generate-plan + ag-test-coverage-plan. Do NOT invoke ag-generate-phase-program. Do NOT use agent-team. DO invoke ag-agent-strategy-compare. Scope fence: the new plan covers only the conflict-resolution surface.
...
- **Blast-radius conflict still unresolved after 1 re-scope cycle** — surface a structured summary showing (a) which phases conflict, (b) which files, (c) resolutions tried — then await the user's decision before spawning outer ag-validate-agent.
...
BLOCKED-skip variant: when ag-validate-agent emits `PHASE_SKIPPED: BLOCKED`, orchestrator marks Step 4 as:
...
When ag-plan-agent creates a new plan mid-program (for conflict resolution or phase insertion), it emits: `MID_PROGRAM_PLAN_CREATED: [plan file path] — inner PVL required`
...
| Template | Path |
|---|---|
| Umbrella plan | `ag-generate-phase-program/templates/umbrella-plan-template.md` |
| Phase stub | `ag-generate-phase-program/templates/phase-stub-template.md` |
| Program Goal Charter | `ag-generate-phase-program/references/program-goal-charter-template.md` |
| Program SPEC (umbrella product-discovery requirements doc governing inner phases) | `ag-generate-phase-program/templates/program-spec-template.md` |
| Phase loop workflow | `ag-generate-phase-program/templates/phase-loop-workflow-template.js` |
