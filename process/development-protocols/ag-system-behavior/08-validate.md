---
name: protocol:ag-system-behavior-08-validate
...
**Agent:** `ag-validate-agent` (sonnet)

VALIDATE is planning/analysis work, not source-code execution, so it runs on sonnet. Only phases that carry out real code or build execution (EXECUTE) run on opus. See the model-selection policy in `.claude/skills/ag-agent-strategy-compare/SKILL.md` §Model Selection Policy.
...
PVL loop:
  V1 → V2 → V3 → V4 → V5 → V6 → V7
                        ...
                        CONDITIONAL → plan-validate-fix loop → back to V1
                          └─ if ag-plan-agent NEEDS_CONTEXT (partial): backlog NOTE + partial supplement + back to V1
...
In **auto-run mode** (ag-autoresearch behavior): the orchestrator automatically routes back to ag-plan-agent on CONDITIONAL/BLOCKED, then back to ag-validate-agent (V1), per iteration without pausing.
In **step-by-step mode**: the orchestrator pauses after each V7 verdict and after each supplement before re-running.

The loop automatically routes back to ag-plan-agent each time PVL finds a gap. After the plan fix, PVL re-runs from V1. This continues until PASS, HALT_PLATEAU, or 10-cycle cap.

**Plan-validate-fix loop:** When V7 returns CONDITIONAL or BLOCKED:

1. ag-plan-agent (supplement mode) appends or updates checklist sections to address the flagged gaps.
...
**This loop is run by the `ag-autoresearch` skill** (`domain: plan`) as its bookkeeper —
autoresearch owns the iteration counter, plateau/regression detection, the per-iteration
report, and the 10-cycle cap; ag-validate-agent and ag-plan-agent keep their own gate and
supplement mechanics. See `ag-autoresearch` SKILL.md §PVL Wiring.

**Parallel fix agents:** The validate step already fans out (ag-validate-findings Layer 1 +
...
Section 8 is the canonical owner of this format. ag-plan-agent.md and ag-validate-agent.md reference this definition — do not redefine it independently.
...
- **ag-intent-clarify** — restate what is being validated; do **NOT** pause here. The restatement + any clarifying questions feed the single entry **Combined Clarification Gate** (`03-session-start.md` Step 6.5), not a separate go-ahead pause.
- **ag-context-discovery** (Part A + B) — load context; must do first
- **ag-plan-discovery** (alongside) — find related plans; must do first
- **ag-review-situation** — confirm active plan and branch
- **ag-agent-strategy-compare** — set strategy for this PVL pass

### Tier-1 Skill

- **ag-scout** — verify all file paths in the plan exist on disk; hard stop if critical paths are missing. (This is a broken-precondition halt, not a safety pause; it halts even under /goal — a plan referencing non-existent critical paths cannot be validated.)
...
**Advisory:** The orchestrator's pre-routing check is advisory only. Always spawn ag-validate-agent and let V1 make the final decision via a fresh file scan. Never skip spawning based on cached state.

### /goal Auto-Proceed Rule

Under /goal: scan the plan file for `## Inner Loop Refresh Note` with a date newer than `## Validate Contract`.

- If present → continue with V1 checks, then proceed to V2. Do not skip V1 structural validation (validate-plan-artifact.mjs, ag-scout path check, ag-review-situation) — those checks run first, then proceed to V2.
...
- **ag-agent-strategy-compare** — run before spawning any validate agents. Input: N Layer-1 agents + M Layer-2 agents. Output: sequential / parallel / workflow / team recommendation.
- **ag-validate-findings** (Tier 1) — run and pass the plan file path. This skill runs both layers.
...
| Test coverage    | Realistic tier strategy; checks whether ag-test-coverage-plan was invoked and tier assignments exist for all blast-radius areas | all-tests.md routing chain                |
| Breaking changes | API contracts, schemas, public contracts, downstream consumers                                                                  | Plan Public Contracts + Blast Radius      |
| Security surface | STRIDE/OWASP via ag-security internally                                                                                         | Invokes ag-security internally            |
...
For each CONCERN: invoke ag-scenario. For high-risk items: invoke ag-predict.
...
**Mandatory orchestrator action:** The orchestrator MUST spawn `ag-debugger` to conduct the empirical probe — it MUST NOT attempt to resolve the probe itself or treat the verdict as known without running ag-debugger. The emitting Layer 2 agent has halted and cannot self-emit the verdict. The sequence is always: probe signal → ORCHESTRATOR spawns ag-debugger → ag-debugger emits `VC-FEASIBILITY-VERDICT-READY` → orchestrator re-spawns ag-validate-agent with the verdict block. Skipping the ag-debugger spawn is a routing gap regardless of whether the orchestrator believes the answer is obvious.
...
**Multiple probes in one V2 pass:** orchestrator batches them; resolves (parallel where cost-class permits); re-spawns ag-validate-agent ONCE with multiple Prior Feasibility: blocks.
...
**Re-spawn entry:** re-spawned ag-validate-agent starts from V1 with Prior Feasibility: block(s) in context. Records resolved probes in a `## Feasibility Probes Resolved` subsection of the validate-contract (omitted when no probe ran).
...
- **ag-sequential-thinking** (Tier 1) — required when agents return conflicting verdicts
- **ag-test-coverage-plan** (Tier 1) — generates Section III: tier assignments per blast-radius area (C1 GAP)

ag-test-coverage-plan must run the full Part A context loading sequence (find tests/ context files, load all-tests.md chain, discover existing test files) before generating tier assignments. It must also emit infra improvement suggestions for any agent-probe or known-gap tier.
...
- ag-sequential-thinking is required when this conflict resolution rule would produce a BLOCKED verdict that Layer 2 alone would not have produced.
...
- **ag-agent-strategy-compare** (Tier 0) — generates the execution strategy section (7-signal table + 4 options) for the next phase (EXECUTE)
...
If the path is BLOCKED: invoke ag-problem-solving first to check whether the FAILs can be resolved.
...
If ag-plan-agent returns NEEDS_CONTEXT during a validate-fix loop (an out-of-scope gap):
...
This section is written exclusively by ag-validate-agent (V7 NEEDS_CONTEXT sub-case). ag-plan-agent in any mode must not modify, remove, or overwrite it. If a SUPPLEMENT REQUEST would require changes in the same location, halt and flag rather than overwriting.
...
- **ag-agent-strategy-compare** (Tier 0) — required — generate EXECUTE phase strategy (full 4-option suite with cost estimates)

### Verdict Paths

**PASS:**

Post-PASS hardening decision is required. The orchestrator reviews all validate findings — even non-blocking ones — and explicitly decides: "Are there concerns, edge cases, or recommendations that would strengthen the plan before execution?" If yes → spawn ag-plan-agent to harden checklist items → route to EXECUTE with the strengthened plan. If no → route to EXECUTE directly.

PASS means "no blockers found." It does not mean "plan is as strong as it could be."

**CONDITIONAL:**

Trigger a plan-validate-fix loop: ag-plan-agent supplements the checklist → PVL re-runs from V1.
...
After ag-plan-agent completes a V7-triggered validate-fix loop, it emits: `SUPPLEMENT_APPLIED: [plan path] — [N] gap(s) addressed`. Orchestrator reaction: PVL re-runs from V1.
...
If ag-plan-agent receives a SUPPLEMENT REQUEST with missing required pipe-delimited fields for any Gap entry: return `NEEDS_CONTEXT: SUPPLEMENT REQUEST malformed — Gap [N] missing [field]. Re-issue structured gap list.` Do not attempt to parse malformed input.
...
After a CONDITIONAL V7 where the user accepts concerns rather than supplementing, the orchestrator surfaces a gap-analysis pass showing validate concerns NOT reflected in the plan checklist — gap analysis distinct from ag-review-situation, currently performed inline.
...
ag-validate-agent writes the block to the plan file but does not auto-print it. The "print the /goal block for copy-paste?" prompt is **not** a separate pause — it is surfaced as part of the single V4 exit gate (Accept → advance to EXECUTE). Under an active /goal: skip it entirely and auto-print.
...
- ag-agent-strategy-compare run for EXECUTE
