---
name: protocol:ag-system-behavior-09-execute
...
**[E-S0] ag-intent-clarify** (Tier 0) — Required first.
...
**[E-S1] ag-context-discovery** (Tier 0) — Required.

Load context. Part A (directory) + Part B (frontmatter → Context Envelope).

**[E-S2] ag-plan-discovery** (Tier 0) — Required.

Run alongside E-S1. Find related plans.

**[E-S3] ag-review-situation** (Tier 0) — Required.
...
**[E-S4] ag-agent-strategy-compare** (Tier 0) — Required.
...
**[E1] ag-scout** (Tier 1) — Required before any breaking change.

Scan the import chain to determine blast radius before modifying a file that other packages import.

**[E2] ag-sequential-thinking** (Tier 1) — Required when plan step ordering is unclear.

Use this to resolve step ordering before proceeding.

**[E3] ag-docs-seeker** (Tier 1) — Required when any library API call signature is unclear.

Do not guess API signatures.

**[E4] ag-scenario** (Tier 2) — Required after implementing any new function, endpoint, or export.

Run before running tests. Surface edge cases before they become test failures.

**[E5] ag-security** (Tier 2) — Required before marking any high-risk section complete.

High-risk sections are: auth, billing, container, secrets, public API.

**[E6] ag-problem-solving** (Tier 2) — Required before escalating BLOCKED.

Exhaust problem-solving techniques before returning to PLAN mode.

**[E7] ag-agent-strategy-compare** (Tier 0) — Required at the end of each major section.
...
In **auto-run mode** (ag-autoresearch behavior): the orchestrator automatically routes back to ag-execute-agent on gate failure, then re-runs tester gates without pausing.
In **step-by-step mode**: the orchestrator pauses after each gate run result and after each execute-fix before re-running.

The loop automatically routes back to ag-execute-agent each time EVL finds a failing gate. After the fix, gates re-run from the start. This continues until all gates pass, HALT_PLATEAU, or 10-cycle cap.

**This loop is run by the `ag-autoresearch` skill** (`domain: tests`, `verify:` = the
validate-contract gate commands) as its bookkeeper — autoresearch owns the iteration
counter, plateau/regression detection, the TSV log, and the 10-cycle cap; ag-tester and
ag-execute-agent keep their own gate-running and supplement mechanics. See `ag-autoresearch`
SKILL.md §EVL Wiring.
...
```
EVL Steps:
  1. Invoke ag-generate-closeout → 9-field closeout packet
     (9-field packet definition: see Section 7 §ag-generate-closeout — 9 Required Fields. All 9 fields are required. The preliminary packet from EVL Step 1 is superseded by the authoritative U-S5 closeout at UPDATE PROCESS entry.)
  ...
  4. If gates NOT green → execute-validate-fix loop:
     a. Spawn ag-execute-agent (supplement mode) with exact failing gate + relevant plan section
  ...
  6. Route to UPDATE PROCESS with full RIPER-5 handoff.
     ...
     Commit timing:
     (1) Source commit fires HERE — orchestrator invokes ag-git-manager immediately after the EVL HANDOFF SUMMARY block is emitted. Includes implementation files only.
...
The orchestrator emits this block (not execute-agent or ag-tester) before closing step 6.
...
context_partial: ["area1", "area2"]  # (bare strings, not bracket-wrapped; matches ag-tester.md output format)
...
If EVL misses a cross-phase regression, the next phase's inner RESEARCH will catch it during Step 1 (which reads prior phase reports and runs ag-test-coverage-plan).
...
| Agent | When to spawn |
|---|---|
| ag-tester | After each implementation sub-step |
| ag-debugger | Bug encountered during implementation |
| ag-code-reviewer | Before marking phase complete |
| ag-code-simplifier | After reviewer passes (optional) |
| ag-ui-ux-designer | UI/UX implementation tasks |
| ag-git-manager | Git operations |

**Write tests on demand rule:** When ag-tester or ag-code-reviewer recommends new tests and the recommendation is within the blast radius of the current plan, execute-agent must write those tests before marking the section complete. Do not defer test-writing to a follow-up stub when the tests are directly verifiable within the current scope.
