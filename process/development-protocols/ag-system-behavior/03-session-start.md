---
name: protocol:ag-system-behavior-03-session-start
...
### Step 3 — ag-context-discovery (Tier 0)
...
| `session-goal`          | If plan has `## Session Goal` → use that (mark `(from plan)`). If no plan yet → use ag-intent-clarify restatement (mark `(from restatement)`)                |
| `blast-radius-packages` | If plan has `## Blast Radius` → use that. If no plan → derive from ag-scout output (mark `(estimated — no plan yet)`). If ag-scout not run → write `PENDING` |
...
### Step 4 — ag-plan-discovery (Tier 0, run alongside Step 3)
...
### Step 5 — ag-review-situation (Tier 0)
...
### Step 6 — ag-agent-strategy-compare (Tier 0)
...
- Dimension 6 (Autonomy Boundaries) from `ag-intent-clarify` is treated as CRITICAL.
...
### Step 7 — Route to ag-research-agent
...
- ag-research-agent spawned with context files and the confirmed strategy attached
