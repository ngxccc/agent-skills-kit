...
## 5. Mandatory Harness Audit & Governance Verification

Before finalizing any task or declaring feature completion, agents MUST execute the parallel audit suite:

```bash
# 1. Validate doc standards and ADR format
bun run .claude/skills/ag-docs/scripts/validate-docs.mjs

# 2. Run the 12-check parallel harness audit
./scripts/run-audit-parallel.mjs
```

Ensure all audit validators pass with **0 failures**:
1. Agent Parity
2. Skills Structure
3. Guide Sync
4. Protocol Wiring
