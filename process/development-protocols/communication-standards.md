...
| validate-contract | The written gate checklist that ag-validate-agent produces before EXECUTE runs. |
...
| Clarification questions (ag-intent-clarify) | **Always** | Lead with the recommended option. |
...
Pointed-to from: `CLAUDE.md`, `AGENTS.md`, all 15 agent files, and all 32 skill files (`ag-generate-closeout`, `ag-intent-clarify`, `ag-review-situation`, `ag-generate-plan`, and 28 others). The per-session enforcement mechanism in Claude Code is a personal **output style** (`~/.claude/output-styles/answer-first.md`), which injects this voice into the system prompt every turn.
