...
_Feito por engenheiros de classe mundial, para agent-skillsrs na_<br>
_[flowser.ai](https://flowser.ai) — Agentes de IA com computadores para GTM_
...
_Esse meta harness transforma qualquer agente de codigo AI num time de engenharia orientado a specs que pesquisa, planeja, entrega codigo production-grade, e auto-aprimora sua memoria pra sobreviver ao apodrecimento de contexto mesmo 6 meses depois._
...
<p>
  ...
  <br>
  <p><em>Nao e apenas decorativo. Ao executar <code>ag-setup</code>, agentes paralelos escaneiam sua base de codigo,<br>
...
└── process/                 # 🧠 Criado pelo ag-setup (nao pelo install)
    └── ...
```

- **Projeto novo?** Instala o harness completo, depois o `ag-setup` estuda seu codebase
- **Ja tem `.claude/` configurado?** Faz backup em `.agent-skills-backup/`, instala do zero, restaura seu `settings.json`
- **Ja tem diretorio `process/`?** Nunca e tocado pelo install — o `ag-setup` cuida da migracao de forma inteligente
- **Ja tem `CLAUDE.md`?** Faz backup como `CLAUDE.md.pre-agent-skills`, instala a versao do harness
...
O agente nao so le seu codigo — ele **estuda outros repositorios** pra aprender como resolveram problemas similares (`ag-xia`).
...
| Funcionalidade                  | agent-skills-kit                                                               | Superpowers                   | GSD                        | gstack                |
| ------------------------------- | ------------------------------------------------------------------------------ | ----------------------------- | -------------------------- | --------------------- |
...
Antes de rotear qualquer request, escaneia <strong>32 skills</strong> e faz match por keywords. Diga "add webhook support" e <code>ag-security</code> + <code>ag-scenario</code> aparecem automaticamente.
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
| --------------------- | ------------------------------------------------------ | ----------------------------------------- |
| 🔍 **RESEARCH**       | Coleta de fatos somente leitura — codebase + web       | _(automatico em feature requests)_        |
...
| 🧠 **UPDATE PROCESS** | Captura aprendizados, atualiza contexto, arquiva plano | _(recomendado apos trabalho nao-trivial)_ |
...
→ Descoberta de skills traz: ag-scenario, ag-security
...
→ Roteia pro ag-debugger → coleta de evidencias → hipoteses concorrentes
...
```
# 🔄 Otimizacao autonoma
Voce: "improve test coverage to 80% using ag-autoresearch"
...
<code>ag-predict</code> — Arquiteto, Seguranca, Performance, UX e Advogado do Diabo debatem seu plano. Produz um veredito <strong>GO / CAUTION / STOP</strong> antes de voce escrever uma linha de codigo.
</td>
<td width="50%" valign="top">
<h1>🎲</h1>
<strong>Gerador de Edge Cases em 12 Dimensoes</strong><br><br>
<code>ag-scenario</code> — Decompoe qualquer feature em 12 dimensoes (tipos de usuario, extremos de input, timing, escala, estado, ambiente, erros, auth, dados, integracoes, compliance, logica de negocio). Outputs utilizaveis como specs de teste.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-security</code> — Auditoria de seguranca com dupla metodologia, auditoria de dependencias, deteccao de secrets, e <strong>modo auto-fix</strong> que ordena por severidade e corrige Critical primeiro com guards de regressao.
</td>
</tr>
</table>
...
<code>ag-autoresearch</code> — Defina um objetivo, va embora. Loop iterativo com backup em git: faz UMA mudanca atomica → commit → mede → mantem ou reverte. Deteccao de travamento apos 5 descartes consecutivos dispara mudancas de estrategia.
</td>
<td width="50%" valign="top">
<h1>👥</h1>
<strong>Times de Agentes em Paralelo</strong><br><br>
<code>ag-team</code> — Multiplos agentes trabalhando <strong>simultaneamente</strong> com isolamento via git worktree. Pesquisa em paralelo, executa em paralelo, revisa em paralelo, debugga de forma adversarial.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-debugger</code> — Coleta evidencias primeiro → forma 2-3 hipoteses concorrentes → testa sistematicamente cada uma → documenta o caminho de eliminacao. <strong>Nunca chuta — prova.</strong>
</td>
</tr>
</table>
...
> 💡 Daqui a seis meses, quando alguem perguntar _"por que construimos auth assim?"_, a resposta ta em `completed/`. Nao perdida numa thread do Slack.
...
| 🧪 **Auditavel**     | `ag-audit-context` verifica roteamento e consistencia                 |
...
| Agente                       | Papel                                                                          |
| ---------------------------- | ------------------------------------------------------------------------------ |
| 🔍 `ag-research-agent`       | Pesquisa de codebase + web, somente leitura. Tracking de contradicoes embutido |
| 💡 `ag-innovate-agent`       | Brainstorm de 2-3 abordagens. Deve produzir resumo de decisao antes do PLAN    |
| 📋 `ag-plan-agent`           | Escreve spec com guards anti-racionalizacao. "Eu ja sei como" nao e um plano   |
| ⚡ `ag-execute-agent`        | Implementa conforme plano. Check-in de 50%, protocolo de desvio, self-review   |
| ⏩ `ag-fast-mode-agent`      | RESEARCH→INNOVATE→PLAN comprimido com pausa de seguranca obrigatoria           |
| 🧠 `ag-update-process-agent` | Checklist obrigatorio de 7 fases incluindo scan de artefatos obsoletos         |
...
| Agente                  | Papel                                                                              |
| ----------------------- | ---------------------------------------------------------------------------------- |
| 🐛 `ag-debugger`        | Evidencia antes de hipotese. Hipoteses concorrentes, cadeias de eliminacao         |
| 🧪 `ag-tester`          | Diff-aware. So roda testes afetados. Auto-escala em mudancas de config             |
| 🔎 `ag-code-reviewer`   | Scout de edge cases ANTES da revisao. Deteccao de N+1, validacao de paths de auth  |
| ✨ `ag-code-simplifier` | Refatoracao de clareza sem mudanca de comportamento                                |
| 🎨 `ag-ui-ux-designer`  | Frontend design-aware. Pode spawnar subagent de pesquisa durante execucao          |
| 📦 `ag-git-manager`     | Split logico de commits a partir de `touched_files`. Recusa arquivos desconhecidos |
...
**🧠 Planejamento** — `ag-predict` (debate com 5 personas) · `ag-scenario` (edge cases em 12 dimensoes) · `ag-sequential-thinking` · `ag-problem-solving`

**🐛 Debug & seguranca** — `ag-debug` · `ag-security` (STRIDE + OWASP + auto-fix) · `ag-autoresearch` (otimizacao autonoma)

**📚 Pesquisa** — `ag-docs-seeker` · `ag-scout` · `ag-docs` · `ag-repomix` · `ag-xia` (comparacao de repos)

**🎨 Frontend** — `ag-frontend-design` · `ag-chrome-devtools` · `ag-agent-browser` · `ag-web-testing`

**⚙️ Utilitarios** — `ag-context-engineering` · `ag-mcp-management` · `ag-preview` · `ag-team` (agentes em paralelo) · `ag-tech-graph` · `ag-watzup` (handoff de sessao) · `ag-merge-worktree`

</details>

> 💡 Algumas skills (como `ag-xia`) foram inspiradas pelo [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G) de [@mrgoonie](https://github.com/mrgoonie). Focamos em menos skills, porém mais profundas, ao invés de 80+.
...
- 🐛 [Reportar um bug](https://github.com/ngxccc/agent-skills-kit/issues/new?template=1.bug_report.yml)
- 💡 [Solicitar uma feature](https://github.com/ngxccc/agent-skills-kit/issues/new?template=2.feature_request.yml)
- ⚡ [Enviar uma skill](https://github.com/ngxccc/agent-skills-kit/issues/new?template=3.skill_submission.yml)
- 🌐 [Adicionar uma traducao](https://github.com/ngxccc/agent-skills-kit/issues/new?template=5.translation.yml)
...
Este projeto é clonado e personalizado a partir do [vibecode-pro-max-kit](https://github.com/withkynam/vibecode-pro-max-kit) por [@withkynam](https://github.com/withkynam).
...
A diferença: agent-skills-kit foca no framework de desenvolvimento spec-driven e organização de contexto auto-aprimorável, sem inchar com 80+ skills. Menos ferramentas, mais estrutura.
