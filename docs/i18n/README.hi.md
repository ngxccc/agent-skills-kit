...
<p>
  ...
  <br>
  <p><em>यह केवल सजावट नहीं है। जब आप <code>ag-setup</code> चलाते हैं, तो parallel agents आपके codebase को scan करते हैं,<br>
...
└── process/                 # 🧠 ag-setup द्वारा बनाया जाता है (install से नहीं)
    └── ...
```

- **नया project?** पूरा harness install होता है, फिर `ag-setup` आपका codebase study करती है
- **पहले से `.claude/` config है?** `.agent-skills-backup/` में backup होता है, fresh install होता है, आपका `settings.json` restore होता है
- **पहले से `process/` directory है?** Install कभी छूती नहीं — `ag-setup` intelligently migration handle करती है
- **पहले से `CLAUDE.md` है?** `CLAUDE.md.pre-agent-skills` के रूप में backup होता है, harness version install होता है
...
Agent सिर्फ आपका code नहीं पढ़ता — यह **दूसरे repositories study करता है** यह जानने के लिए कि उन्होंने similar problems कैसे solve किए (`ag-xia`)।
...
किसी भी request को route करने से पहले, <strong>32 skills</strong> scan करता है और keywords match करता है। "add webhook support" कहें और <code>ag-security</code> + <code>ag-scenario</code> automatically surface हो जाते हैं।
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
→ Skill discovery surface करता है: ag-scenario, ag-security
...
→ ag-debugger को route → evidence gathering → competing hypotheses
...
आप: "ag-autoresearch का उपयोग करके test coverage 80% तक improve करो"
...
<code>ag-predict</code> — Architect, Security, Performance, UX, और Devil's Advocate आपके plan पर debate करते हैं। Code की एक लाइन लिखने से पहले <strong>GO / CAUTION / STOP</strong> verdict देता है।
</td>
<td width="50%" valign="top">
<h1>🎲</h1>
<strong>12-Dimension Edge Case Generator</strong><br><br>
<code>ag-scenario</code> — किसी भी feature को 12 dimensions में decompose करता है (user types, input extremes, timing, scale, state, env, errors, auth, data, integrations, compliance, business logic)। Output test specs के रूप में उपयोग करने योग्य।
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-security</code> — Dependency auditing, secret detection, और <strong>auto-fix mode</strong> के साथ dual-methodology security audit जो severity से sort करता है और पहले Critical को regression guards के साथ fix करता है।
</td>
</tr>
</table>
...
<code>ag-autoresearch</code> — Goal set करें, चले जाएं। Iterative git-backed loop: ONE atomic change करें → commit → measure → keep या revert। 5 consecutive discards के बाद stuck detection strategy shifts trigger करता है।
</td>
<td width="50%" valign="top">
<h1>👥</h1>
<strong>Parallel Agent Teams</strong><br><br>
<code>ag-team</code> — Multiple agents <strong>simultaneously</strong> git worktree isolation के साथ काम करते हैं। Parallel में research, parallel में execute, parallel में review, adversarially debug।
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-debugger</code> — पहले evidence gather करता है → 2-3 competing hypotheses बनाता है → systematically हर एक test करता है → elimination path document करता है। <strong>कभी guess नहीं करता — prove करता है।</strong>
</td>
</tr>
</table>
...
> 💡 कुछ skills (जैसे `ag-xia`) [@mrgoonie](https://github.com/mrgoonie) के [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G) से inspired हैं। हमने 80+ की बजाय कम, गहरी skills पर focus किया।
...
- 🐛 [Bug report करें](https://github.com/ngxccc/agent-skills-kit/issues/new?template=1.bug_report.yml)
- 💡 [Feature request करें](https://github.com/ngxccc/agent-skills-kit/issues/new?template=2.feature_request.yml)
- ⚡ [Skill submit करें](https://github.com/ngxccc/agent-skills-kit/issues/new?template=3.skill_submission.yml)
- 🌐 [Translation add करें](https://github.com/ngxccc/agent-skills-kit/issues/new?template=5.translation.yml)
...
यह project [@withkynam](https://github.com/withkynam) के [vibecode-pro-max-kit](https://github.com/withkynam/vibecode-pro-max-kit) से clone और customize किया गया है।
...
अंतर यह है: agent-skills-kit spec-driven development framework और self-improving context organization पर focus करता है, बिना आपको 80+ skills से overwhelm किए। कम tools, ज़्यादा structure।
