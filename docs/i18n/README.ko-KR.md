...
_세계적 수준의 엔지니어들이 바이브코더를 위해 만들었어요_<br>
_[flowser.ai](https://flowser.ai) — GTM을 위한 컴퓨터 기반 AI 에이전트_
...
_이 meta harness는 AI 코딩 에이전트를 리서치하고, 계획 세우고, 프로덕션급 코드를 출시하고, 6개월이 지나도 컨텍스트 부패에서 살아남도록 스스로 메모리를 개선하는 스펙 기반 엔지니어링 팀으로 바꿔줘요._
...
<p>
  ...
  <br>
  <p><em>단순 장식이 아닙니다. <code>ag-setup</code>을 실행하면 병렬 에이전트가 코드베이스를 스캔하고,<br>
...
└── process/                 # 🧠 ag-setup이 생성 (install이 아님)
    └── ...
```

- **새 프로젝트?** 전체 harness를 설치한 후, `ag-setup`이 코드베이스를 분석해요
- **기존 `.claude/` 설정이 있다면?** `.agent-skills-backup/`에 백업하고, 새로 설치한 뒤, `settings.json`을 복원해요
- **기존 `process/` 디렉토리가 있다면?** install은 절대 건드리지 않아요 — `ag-setup`이 마이그레이션을 똑똑하게 처리해요
- **기존 `CLAUDE.md`가 있다면?** `CLAUDE.md.pre-agent-skills`로 백업하고, harness 버전을 설치해요
...
에이전트가 단순히 여러분의 코드만 읽는 게 아니라 — 비슷한 문제를 어떻게 해결했는지 **다른 레포지토리를 연구**해요 (`ag-xia`).
...
| 기능                   | agent-skills-kit                                              | Superpowers           | GSD                | gstack         |
| ---------------------- | ------------------------------------------------------------- | --------------------- | ------------------ | -------------- |
...
요청을 라우팅하기 전에 <strong>32개 skill</strong>을 스캔하고 키워드를 매칭해요. "add webhook support"라고 하면 <code>ag-security</code> + <code>ag-scenario</code>가 자동으로 표시돼요.
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
<code>ag-predict</code> — Architect, Security, Performance, UX, Devil's Advocate가 플랜을 토론해요. 코드 한 줄 쓰기 전에 <strong>GO / CAUTION / STOP</strong> 판정을 내려요.
</td>
<td width="50%" valign="top">
<h1>🎲</h1>
<strong>12차원 엣지 케이스 생성기</strong><br><br>
<code>ag-scenario</code> — 모든 기능을 12개 차원(사용자 유형, 입력 극단값, 타이밍, 규모, 상태, 환경, 에러, 인증, 데이터, 통합, 규정, 비즈니스 로직)에 걸쳐 분해해요. 출력은 테스트 스펙으로 바로 사용 가능해요.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-security</code> — 의존성 감사, 시크릿 탐지가 포함된 이중 방법론 보안 감사에요. Critical부터 수정하고 리그레션 가드가 있는 <strong>자동 수정 모드</strong>가 있어요.
</td>
</tr>
</table>
...
<code>ag-autoresearch</code> — 목표를 설정하고 자리를 비우세요. 반복적인 git-backed 루프: 하나의 atomic 변경 → 커밋 → 측정 → 유지 또는 되돌리기. 5번 연속 폐기 후 stuck 감지가 전략 전환을 트리거해요.
</td>
<td width="50%" valign="top">
<h1>👥</h1>
<strong>병렬 에이전트 팀</strong><br><br>
<code>ag-team</code> — 여러 에이전트가 git worktree 격리로 <strong>동시에</strong> 작업해요. 병렬 리서치, 병렬 실행, 병렬 리뷰, 적대적 디버깅.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-debugger</code> — 먼저 증거를 수집하고 → 2-3개의 경쟁 가설을 세우고 → 각각을 체계적으로 테스트하고 → 소거 경로를 문서화해요. <strong>절대 추측하지 않아요 — 증명해요.</strong>
</td>
</tr>
</table>
...
| 🧪 **감사 가능**     | `ag-audit-context`가 라우팅과 일관성을 검증해요                |
...
| Agent                        | 역할                                                                |
| ---------------------------- | ------------------------------------------------------------------- |
| 🔍 `ag-research-agent`       | 코드베이스 + 웹 리서치, 읽기 전용. 모순 추적 내장                   |
| 💡 `ag-innovate-agent`       | 2-3가지 접근 방식 브레인스토밍. PLAN 전에 결정 요약 필수            |
| 📋 `ag-plan-agent`           | 합리화 방지 가드와 함께 스펙 작성. "이미 알고 있어요"는 플랜이 아님 |
| ⚡ `ag-execute-agent`        | 플랜대로 구현. 50% 체크인, 이탈 프로토콜, 셀프 리뷰                 |
| ⏩ `ag-fast-mode-agent`      | 압축된 RESEARCH→INNOVATE→PLAN에 필수 안전 일시정지                  |
| 🧠 `ag-update-process-agent` | stale 산출물 스캐닝을 포함한 7단계 필수 체크리스트                  |
...
| Agent                   | 역할                                                               |
| ----------------------- | ------------------------------------------------------------------ |
| 🐛 `ag-debugger`        | 증거 우선 가설. 경쟁 가설, 소거 체인                               |
| 🧪 `ag-tester`          | Diff 인식. 영향 받는 테스트만 실행. 설정 변경 시 자동 에스컬레이션 |
| 🔎 `ag-code-reviewer`   | 리뷰 전 엣지 케이스 스카우트. N+1 탐지, 인증 경로 검증             |
| ✨ `ag-code-simplifier` | 동작 변경 없는 가독성 리팩토링                                     |
| 🎨 `ag-ui-ux-designer`  | 디자인 인식 프론트엔드. 실행 중 리서치 subagent 생성 가능          |
| 📦 `ag-git-manager`     | `touched_files`에서 논리적 커밋 분할. 알 수 없는 파일은 거부       |
...
**🔧 Contract skill** — `ag-generate-plan` · `ag-generate-context` · `ag-audit-context` · `ag-audit-plans` · `ag-audit-ag` · `ag-setup` · `ag-update` · `ag-publish`

**🧠 Planning** — `ag-predict` (5-페르소나 토론) · `ag-scenario` (12차원 엣지 케이스) · `ag-sequential-thinking` · `ag-problem-solving`

**🐛 Debug & security** — `ag-debug` · `ag-security` (STRIDE + OWASP + 자동 수정) · `ag-autoresearch` (자율 최적화)

**📚 Research** — `ag-docs-seeker` · `ag-scout` · `ag-docs` · `ag-repomix` · `ag-xia` (레포 비교)

**🎨 Frontend** — `ag-frontend-design` · `ag-chrome-devtools` · `ag-agent-browser` · `ag-web-testing`

**⚙️ Utilities** — `ag-context-engineering` · `ag-mcp-management` · `ag-preview` · `ag-team` (병렬 에이전트) · `ag-tech-graph` · `ag-watzup` (세션 핸드오프) · `ag-merge-worktree`

</details>

> 💡 일부 스킬(`ag-xia` 등)은 [@mrgoonie](https://github.com/mrgoonie)의 [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G)에서 영감을 받았습니다. 80개 이상 대신 더 적고 깊은 스킬에 집중했습니다.
...
- 🐛 [버그 신고](https://github.com/ngxccc/agent-skills-kit/issues/new?template=1.bug_report.yml)
- 💡 [기능 요청](https://github.com/ngxccc/agent-skills-kit/issues/new?template=2.feature_request.yml)
- ⚡ [Skill 제출](https://github.com/ngxccc/agent-skills-kit/issues/new?template=3.skill_submission.yml)
- 🌐 [번역 추가](https://github.com/ngxccc/agent-skills-kit/issues/new?template=5.translation.yml)
...
이 프로젝트는 [@withkynam](https://github.com/withkynam)의 [vibecode-pro-max-kit](https://github.com/withkynam/vibecode-pro-max-kit)을 클론하고 맞춤 설정한 버전입니다.
...
차이점: agent-skills-kit은 스펙 기반 개발 프레임워크와 자기 개선 컨텍스트 구성에 집중하며, 80개 이상의 스킬로 부풀리지 않습니다. 도구는 적게, 구조는 더 많이.
