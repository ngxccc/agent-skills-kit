...
<p>
  ...
  <br>
  <p><em>不只是装饰。运行 <code>ag-setup</code> 时，并行 agent 会扫描你的代码库，<br>
...
├── AGENTS.md                # 📖 Agent 注册表
└── process/                 # 🧠 由 ag-setup 创建（不是 install）
    └── ...
```

- **新项目？** 安装完整 harness，然后 `ag-setup` 研究你的代码库
- **已有 `.claude/` 配置？** 备份到 `.agent-skills-backup/`，全新安装，恢复你的 `settings.json`
- **已有 `process/` 目录？** install 完全不碰——`ag-setup` 会智能迁移
- **已有 `CLAUDE.md`？** 备份为 `CLAUDE.md.pre-agent-skills`，安装 harness 版本
...
agent 不只是读你的代码——它会**研究其他仓库**学习别人怎么解决类似问题（`ag-xia`）。
...
在路由任何请求之前，扫描 <strong>32 个 skills</strong> 并匹配关键词。你说 "add webhook support"，<code>ag-security</code> + <code>ag-scenario</code> 自动浮现。
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
<code>ag-predict</code> — 架构师、安全专家、性能专家、UX 专家和魔鬼代言人就你的方案展开辩论。在你写第一行代码之前给出 <strong>GO / CAUTION / STOP</strong> 裁定。
</td>
<td width="50%" valign="top">
<h1>🎲</h1>
<strong>12 维度边界情况生成器</strong><br><br>
<code>ag-scenario</code> — 从 12 个维度分解任何功能（用户类型、输入极端值、时序、规模、状态、环境、错误、auth、数据、集成、合规、业务逻辑）。输出可以直接当测试 spec 用。
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-security</code> — 双方法论安全审计，包含依赖审计、密钥检测，还有<strong>自动修复模式</strong>按严重性排序，先修 Critical——每步都有回归防护。
</td>
</tr>
</table>
...
<code>ag-autoresearch</code> — 设定目标，放心走开。迭代式 git 支持的循环：做一个原子变更 → commit → 测量 → 保留或回滚。连续丢弃 5 次后触发卡住检测，自动切换策略。
</td>
<td width="50%" valign="top">
<h1>👥</h1>
<strong>并行 Agent 团队</strong><br><br>
<code>ag-team</code> — 多个 agent <strong>同时</strong>工作，用 git worktree 隔离。并行调研、并行开发、并行 review、对抗式调试。
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-debugger</code> — 先收集证据 → 形成 2-3 个竞争假设 → 系统性逐个验证 → 记录排除路径。<strong>从不猜测——它证明。</strong>
</td>
</tr>
</table>
...
**🧠 规划** — `ag-predict`（5 人格辩论） · `ag-scenario`（12 维度边界情况） · `ag-sequential-thinking` · `ag-problem-solving`

**🐛 调试 & 安全** — `ag-debug` · `ag-security`（STRIDE + OWASP + 自动修复） · `ag-autoresearch`（自主优化）

**📚 调研** — `ag-docs-seeker` · `ag-scout` · `ag-docs` · `ag-repomix` · `ag-xia`（仓库对比）

**🎨 前端** — `ag-frontend-design` · `ag-chrome-devtools` · `ag-agent-browser` · `ag-web-testing`

**⚙️ 工具** — `ag-context-engineering` · `ag-mcp-management` · `ag-preview` · `ag-team`（并行 agents） · `ag-tech-graph` · `ag-watzup`（会话交接） · `ag-merge-worktree`

</details>

> 💡 部分技能（如 `ag-xia`）受 [@mrgoonie](https://github.com/mrgoonie) 的 [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G) 启发。我们专注于更少但更深入的技能，而非 80+。
...
- 🐛 [报告 Bug](https://github.com/ngxccc/agent-skills-kit/issues/new?template=1.bug_report.yml)
- 💡 [功能请求](https://github.com/ngxccc/agent-skills-kit/issues/new?template=2.feature_request.yml)
- ⚡ [提交 Skill](https://github.com/ngxccc/agent-skills-kit/issues/new?template=3.skill_submission.yml)
- 🌐 [添加翻译](https://github.com/ngxccc/agent-skills-kit/issues/new?template=5.translation.yml)
...
本项目克隆自 [@withkynam](https://github.com/withkynam) 的 [vibecode-pro-max-kit](https://github.com/withkynam/vibecode-pro-max-kit) 并进行了定制。

本项目受益于 [@mrgoonie](https://github.com/mrgoonie) 的 [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G)——尤其是 `ck:xia` 等技能给了我们启发。

区别在于：agent-skills-kit 专注于规格驱动的开发框架和自我改进的上下文组织，不会用 80+ 个技能让你应接不暇。更少工具，更多结构。
