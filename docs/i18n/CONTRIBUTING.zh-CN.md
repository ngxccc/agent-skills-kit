...
# 为 agent-skills-kit 做贡献

感谢你有兴趣为 agent-skills-kit 做贡献！这个项目为 Claude Code 和 Codex 提供开箱即用的 agent harness，我们欢迎所有人的贡献。
...
- **不要**使用 `ag-` 前缀——该前缀保留给官方 harness skills
...
仓库根目录的 `ag-manifest.json` 文件追踪 harness 中所有受管文件。当你添加新文件（skills、agents、hooks、protocols、seeds）时，必须更新这个 manifest。

`install.sh` 和 `ag-update` 用这个 manifest 来决定复制和同步哪些文件。如果你的新文件没在 manifest 里，用户安装或更新时就不会包含它。
...
- [ ] Skill 名称**没有**使用 `ag-` 前缀（保留给官方 skills）
- [ ] 辅助脚本放在 `scripts/` 子目录下
- [ ] `ag-manifest.json` 已更新所有新文件路径
- [ ] 验证通过：
