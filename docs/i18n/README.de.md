...
_Entwickelt von Weltklasse-Ingenieuren, für Vibecoders bei_<br>
_[flowser.ai](https://flowser.ai) — KI-Agenten mit Computern für GTM_
...
<p>
  ...
  <br>
  <p><em>Nicht nur zur Dekoration. Wenn du <code>ag-setup</code> ausführst, scannen parallele Agenten deine Codebasis,<br>
...
> **Führe dies in deinem Projektordner aus.** Öffne ein Terminal und wechsle mit `cd` in das Projekt, in das das Harness installiert werden soll, _bevor_ du den Befehl ausführst — es wird in das aktuelle Verzeichnis installiert.
>
...
└── process/                 # 🧠 Erstellt von ag-setup (nicht install)
    └── ...
```

- **Neues Projekt?** Installiert das vollständige Harness, dann studiert `ag-setup` deine Codebasis
- **Vorhandene `.claude/`-Konfiguration?** Wird in `.agent-skills-backup/` gesichert, Harness neu installiert, deine `settings.json` wiederhergestellt
- **Vorhandenes `process/`-Verzeichnis?** Wird von install nie angetastet — `ag-setup` übernimmt die Migration intelligent
- **Vorhandene `CLAUDE.md`?** Wird als `CLAUDE.md.pre-agent-skills` gesichert, Harness-Version installiert
...
Der Agent liest nicht nur deinen Code — er **studiert andere Repositories**, um zu lernen, wie sie ähnliche Probleme gelöst haben (`ag-xia`).
...
Vor dem Routing jeder Anfrage werden <strong>32 Skills</strong> gescannt und Keywords abgeglichen. Sag „Webhook-Unterstützung hinzufügen" und <code>ag-security</code> + <code>ag-scenario</code> erscheinen automatisch.
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
| --------------------- | -------------------------------------------------------------- | ----------------------------------------- |
| 🔍 **RESEARCH**       | Read-only Faktensammlung — Codebasis + Web                     | _(automatisch bei Feature-Anfragen)_      |
...
| 🧠 **UPDATE PROCESS** | Erkenntnisse erfassen, Kontext aktualisieren, Plan archivieren | _(empfohlen nach nicht-trivialer Arbeit)_ |
...
→ Skill-Erkennung zeigt an: ag-scenario, ag-security
...
→ Leitet zu ag-debugger → Beweissammlung → konkurrierende Hypothesen
...
Du: "Testabdeckung mit ag-autoresearch auf 80% verbessern"
...
<code>ag-predict</code> — Architekt, Sicherheit, Performance, UX und Devil's Advocate debattieren deinen Plan. Liefert ein <strong>GO / CAUTION / STOP</strong>-Urteil, bevor du eine Zeile Code schreibst.
</td>
<td width="50%" valign="top">
<h1>🎲</h1>
<strong>12-Dimensions-Edge-Case-Generator</strong><br><br>
<code>ag-scenario</code> — Zerlegt jedes Feature in 12 Dimensionen (Benutzertypen, Eingabe-Extreme, Timing, Skalierung, Zustand, Umgebung, Fehler, Auth, Daten, Integrationen, Compliance, Geschäftslogik). Ausgabe direkt als Testspecs verwendbar.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-security</code> — Dual-Methodik-Sicherheitsaudit mit Abhängigkeits-Auditing, Secret-Erkennung und <strong>Auto-Fix-Modus</strong>, der nach Schweregrad sortiert und Critical zuerst mit Regressions-Guards behebt.
</td>
</tr>
</table>
...
<code>ag-autoresearch</code> — Ziel setzen, weggehen. Iterativer git-basierter Loop: EINE atomare Änderung vornehmen → committen → messen → behalten oder rückgängig machen. Stuck-Erkennung nach 5 aufeinanderfolgenden Verwerfungen löst Strategiewechsel aus.
</td>
<td width="50%" valign="top">
<h1>👥</h1>
<strong>Parallele Agenten-Teams</strong><br><br>
<code>ag-team</code> — Mehrere Agenten arbeiten <strong>gleichzeitig</strong> mit git-Worktree-Isolation. Parallel recherchieren, parallel ausführen, parallel reviewen, adversarial debuggen.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-debugger</code> — Sammelt erst Beweise → bildet 2-3 konkurrierende Hypothesen → testet jede systematisch → dokumentiert den Ausschlusspfad. <strong>Rät nie — beweist.</strong>
</td>
</tr>
</table>
...
> 💡 In sechs Monaten, wenn jemand fragt _„Warum haben wir Auth so gebaut?"_, ist die Antwort in `completed/`. Nicht in einem Slack-Thread verloren.
...
| 🧪 **Auditierbar**    | `ag-audit-context` verifiziert Routing und Konsistenz                      |
...
| Agent                        | Rolle                                                                                |
| ---------------------------- | ------------------------------------------------------------------------------------ |
| 🔍 `ag-research-agent`       | Codebasis- + Web-Recherche, read-only. Widerspruchs-Tracking eingebaut               |
| 💡 `ag-innovate-agent`       | 2-3 Ansätze brainstormen. Muss Entscheidungszusammenfassung vor PLAN liefern         |
| 📋 `ag-plan-agent`           | Spec mit Anti-Rationalisierungs-Guards schreiben. „Ich weiß schon wie" ist kein Plan |
| ⚡ `ag-execute-agent`        | Nach Plan implementieren. 50%-Check-in, Abweichungsprotokoll, Self-Review            |
| ⏩ `ag-fast-mode-agent`      | Komprimiertes RESEARCH→INNOVATE→PLAN mit Pflicht-Sicherheitspause                    |
| 🧠 `ag-update-process-agent` | 7-Phasen-Pflicht-Checkliste inklusive Scan nach veralteten Artefakten                |
...
| Agent                   | Rolle                                                                                            |
| ----------------------- | ------------------------------------------------------------------------------------------------ |
| 🐛 `ag-debugger`        | Beweis-vor-Hypothese. Konkurrierende Hypothesen, Ausschlussketten                                |
| 🧪 `ag-tester`          | Diff-bewusst. Führt nur betroffene Tests aus. Eskaliert automatisch bei Konfigurationsänderungen |
| 🔎 `ag-code-reviewer`   | Edge-Case-Scout VOR dem Review. N+1-Erkennung, Auth-Pfad-Validierung                             |
| ✨ `ag-code-simplifier` | Verständlichkeits-Refactoring ohne Verhaltensänderung                                            |
| 🎨 `ag-ui-ux-designer`  | Design-bewusstes Frontend. Kann Recherche-Subagenten mitten in der Ausführung starten            |
| 📦 `ag-git-manager`     | Logisches Commit-Splitting aus `touched_files`. Lehnt unbekannte Dateien ab                      |
...
**🧠 Planung** — `ag-predict` (5-Persona-Debatte) · `ag-scenario` (12-Dimensions-Edge-Cases) · `ag-sequential-thinking` · `ag-problem-solving`

**🐛 Debug & Sicherheit** — `ag-debug` · `ag-security` (STRIDE + OWASP + Auto-Fix) · `ag-autoresearch` (autonome Optimierung)

**📚 Recherche** — `ag-docs-seeker` · `ag-scout` · `ag-docs` · `ag-repomix` · `ag-xia` (Repo-Vergleich)

**🎨 Frontend** — `ag-frontend-design` · `ag-chrome-devtools` · `ag-agent-browser` · `ag-web-testing`

**⚙️ Hilfsmittel** — `ag-context-engineering` · `ag-mcp-management` · `ag-preview` · `ag-team` (parallele Agenten) · `ag-tech-graph` · `ag-watzup` (Sitzungsübergabe) · `ag-merge-worktree`

</details>

> 💡 Einige Skills (wie `ag-xia`) wurden von [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G) von [@mrgoonie](https://github.com/mrgoonie) inspiriert. Wir haben uns auf weniger, aber tiefere Skills konzentriert statt auf 80+.
...
- 🐛 [Einen Bug melden](https://github.com/ngxccc/agent-skills-kit/issues/new?template=1.bug_report.yml)
- 💡 [Ein Feature anfragen](https://github.com/ngxccc/agent-skills-kit/issues/new?template=2.feature_request.yml)
- ⚡ [Einen Skill einreichen](https://github.com/ngxccc/agent-skills-kit/issues/new?template=3.skill_submission.yml)
- 🌐 [Eine Übersetzung hinzufügen](https://github.com/ngxccc/agent-skills-kit/issues/new?template=5.translation.yml)
