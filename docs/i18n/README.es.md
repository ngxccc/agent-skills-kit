...
_Creado por ingenieros de clase mundial, para agent-skillsrs en_<br>
_[flowser.ai](https://flowser.ai) — Agentes de IA con computadoras para GTM_
...
<p>
  ...
  <br>
  <p><em>No es decorativo. Al ejecutar <code>ag-setup</code>, agentes en paralelo escanean tu codebase,<br>
...
> **Ejecuta esto dentro de la carpeta de tu proyecto.** Abre una terminal y usa `cd` para entrar al proyecto donde quieres instalar el harness _antes_ de ejecutar el comando — se instala en el directorio actual.
>
...
└── process/                 # 🧠 Creado por ag-setup (no por install)
    └── ...
```

- **¿Proyecto nuevo?** Instala el harness completo, luego `ag-setup` estudia tu codebase
- **¿Ya tienes configuración `.claude/`?** Hace backup en `.agent-skills-backup/`, instala desde cero, restaura tu `settings.json`
- **¿Ya tienes directorio `process/`?** Nunca es tocado por install — `ag-setup` maneja la migración de forma inteligente
- **¿Ya tienes `CLAUDE.md`?** Se guarda como `CLAUDE.md.pre-agent-skills`, se instala la versión del harness
...
flowchart TD
    R["🔍 RESEARCH\nLeer codebase, reunir hechos"]
    I["💡 INNOVATE\nExplorar 2-3 enfoques"]
    P["📋 PLAN\nEscribir spec detallada"]
    E["⚡ EXECUTE\nImplementar el plan"]
...
El agente no solo lee tu código — **estudia otros repositorios** para aprender cómo resolvieron problemas similares (`ag-xia`).
...
Antes de enrutar cualquier solicitud, escanea <strong>32 skills</strong> y hace coincidencia de palabras clave. Di "add webhook support" y <code>ag-security</code> + <code>ag-scenario</code> aparecen automáticamente.
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
| --------------------- | --------------------------------------------------------- | --------------------------------------------- |
| 🔍 **RESEARCH**       | Recopilación de hechos de solo lectura — codebase + web   | _(automático en feature requests)_            |
...
| 🧠 **UPDATE PROCESS** | Capturar aprendizajes, actualizar contexto, archivar plan | _(recomendado después de trabajo no trivial)_ |
...
→ El descubrimiento de skills muestra: ag-scenario, ag-security
...
→ Enruta a ag-debugger → recopilación de evidencia → hipótesis concurrentes
...
Tú: "improve test coverage to 80% using ag-autoresearch"
...
<code>ag-predict</code> — Arquitecto, Seguridad, Performance, UX y el Abogado del Diablo debaten tu plan. Produce un veredicto <strong>GO / CAUTION / STOP</strong> antes de que escribas una sola línea de código.
</td>
<td width="50%" valign="top">
<h1>🎲</h1>
<strong>Generador de Edge Cases en 12 Dimensiones</strong><br><br>
<code>ag-scenario</code> — Descompone cualquier feature en 12 dimensiones (tipos de usuario, extremos de entrada, timing, escala, estado, entorno, errores, auth, datos, integraciones, cumplimiento, lógica de negocio). Los outputs son utilizables como specs de prueba.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-security</code> — Auditoría de seguridad con metodología dual, auditoría de dependencias, detección de secretos y <strong>modo auto-fix</strong> que ordena por severidad y corrige Critical primero con guardas de regresión.
</td>
</tr>
</table>
...
<code>ag-autoresearch</code> — Define un objetivo, aléjate. Bucle iterativo respaldado por git: hace UN cambio atómico → commit → mide → conserva o revierte. La detección de bloqueo después de 5 descartes consecutivos activa cambios de estrategia.
</td>
<td width="50%" valign="top">
<h1>👥</h1>
<strong>Equipos de Agentes en Paralelo</strong><br><br>
<code>ag-team</code> — Múltiples agentes trabajando <strong>simultáneamente</strong> con aislamiento mediante git worktree. Investigación en paralelo, ejecución en paralelo, revisión en paralelo, debugging adversarial.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-debugger</code> — Reúne evidencia primero → forma 2-3 hipótesis concurrentes → las prueba sistemáticamente → documenta el camino de eliminación. <strong>Nunca adivina — demuestra.</strong>
</td>
</tr>
</table>
...
flowchart TD
    ...
    T["🧪 Tester\nDiff-aware — solo ejecuta\npruebas afectadas"]
    CR["🔍 Code Reviewer\nScout de edge cases\n+ revisión adversarial"]
    CS["✨ Code Simplifier\nRefactorización de claridad"]
...
flowchart TD
    ...
    B["📝 Plan Creado\nen active/"]
    C{"👀 Usuario Revisa\nel Plan"}
    D["⚡ Ejecutar Contra el Plan"]
    E["📦 Plan Archivado\nen completed/"]
    F["🧠 Aprendizajes Escritos\nen all-context.md"]
    G["🔄 Próxima Feature\nComienza Más Inteligente"]

    A --> B
    B --> C
    C -->|"✅ Aprobado"| D
    C -->|"✏️ Necesita Cambios"| B
    D --> E
    E --> F
    F --> G
    G -.->|"el contexto se acumula"| A
...
> 💡 Seis meses después, cuando alguien pregunte _"¿por qué construimos auth de esta manera?"_, la respuesta está en `completed/`. No perdida en un hilo de Slack.
...
flowchart TD
    ...
    E1["⚡ Ejecutar"]
    V1["✅ Validar"]
    RP1["📄 Reporte Duradero"]

    R2["🔍 Re-Investigación"]
    E2["⚡ Ejecutar"]
    V2["✅ Validar"]
    RP2["📄 Reporte Duradero"]
    ...
    E1 --> V1
    V1 --> RP1
    RP1 -.->|"los aprendizajes alimentan\nla siguiente fase"| R2
...
| 🧪 **Auditable**      | `ag-audit-context` verifica el enrutamiento y la consistencia              |
...
| Agente                       | Rol                                                                                      |
| ---------------------------- | ---------------------------------------------------------------------------------------- |
| 🔍 `ag-research-agent`       | Investigación de codebase + web, solo lectura. Seguimiento de contradicciones integrado  |
| 💡 `ag-innovate-agent`       | Brainstorm de 2-3 enfoques. Debe producir resumen de decisión antes del PLAN             |
| 📋 `ag-plan-agent`           | Escribe spec con guardas anti-racionalización. "Ya sé cómo hacerlo" no es un plan        |
| ⚡ `ag-execute-agent`        | Implementa según el plan. Check-in del 50%, protocolo de desviación, self-review         |
| ⏩ `ag-fast-mode-agent`      | RESEARCH→INNOVATE→PLAN comprimido con pausa de seguridad obligatoria                     |
| 🧠 `ag-update-process-agent` | Lista de verificación obligatoria de 7 fases que incluye escaneo de artefactos obsoletos |
...
| Agente                  | Rol                                                                                            |
| ----------------------- | ---------------------------------------------------------------------------------------------- |
| 🐛 `ag-debugger`        | Evidencia antes de hipótesis. Hipótesis concurrentes, cadenas de eliminación                   |
| 🧪 `ag-tester`          | Diff-aware. Solo ejecuta pruebas afectadas. Escala automáticamente en cambios de configuración |
| 🔎 `ag-code-reviewer`   | Scout de edge cases ANTES de la revisión. Detección de N+1, validación de rutas de auth        |
| ✨ `ag-code-simplifier` | Refactorización de claridad sin cambio de comportamiento                                       |
| 🎨 `ag-ui-ux-designer`  | Frontend consciente del diseño. Puede lanzar subagente de investigación durante la ejecución   |
| 📦 `ag-git-manager`     | Split lógico de commits desde `touched_files`. Rechaza archivos desconocidos                   |
...
**🔧 Skills de contrato** — `ag-generate-plan` · `ag-generate-context` · `ag-audit-context` · `ag-audit-plans` · `ag-audit-ag` · `ag-setup` · `ag-update` · `ag-publish`

**🧠 Planificación** — `ag-predict` (debate con 5 personas) · `ag-scenario` (edge cases en 12 dimensiones) · `ag-sequential-thinking` · `ag-problem-solving`

**🐛 Debug y seguridad** — `ag-debug` · `ag-security` (STRIDE + OWASP + auto-fix) · `ag-autoresearch` (optimización autónoma)

**📚 Investigación** — `ag-docs-seeker` · `ag-scout` · `ag-docs` · `ag-repomix` · `ag-xia` (comparación de repositorios)

**🎨 Frontend** — `ag-frontend-design` · `ag-chrome-devtools` · `ag-agent-browser` · `ag-web-testing`

**⚙️ Utilidades** — `ag-context-engineering` · `ag-mcp-management` · `ag-preview` · `ag-team` (agentes en paralelo) · `ag-tech-graph` · `ag-watzup` (handoff de sesión) · `ag-merge-worktree`

</details>

> 💡 Algunas skills (como `ag-xia`) fueron inspiradas por [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G) de [@mrgoonie](https://github.com/mrgoonie). Nos enfocamos en menos skills pero más profundas en lugar de 80+.
...
- 🐛 [Reportar un bug](https://github.com/ngxccc/agent-skills-kit/issues/new?template=1.bug_report.yml)
- 💡 [Solicitar una feature](https://github.com/ngxccc/agent-skills-kit/issues/new?template=2.feature_request.yml)
- ⚡ [Enviar una skill](https://github.com/ngxccc/agent-skills-kit/issues/new?template=3.skill_submission.yml)
- 🌐 [Agregar una traducción](https://github.com/ngxccc/agent-skills-kit/issues/new?template=5.translation.yml)
...
Este proyecto es un clon y personalización de [vibecode-pro-max-kit](https://github.com/withkynam/vibecode-pro-max-kit) de [@withkynam](https://github.com/withkynam).
...
La diferencia: agent-skills-kit se centra en el framework de desarrollo spec-driven y la organización de contexto auto-mejorable, sin sobrecargarte con 80+ skills. Menos herramientas, más estructura.
