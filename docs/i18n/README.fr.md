...
<p>
  ...
  <br>
  <p><em>Pas décoratif. Quand vous lancez <code>ag-setup</code>, des agents parallèles analysent votre codebase,<br>
...
└── process/                 # 🧠 Créé par ag-setup (pas par install)
    └── ...
```

- **Nouveau projet ?** Installe le harness complet, puis `ag-setup` étudie votre codebase
- **Un `.claude/` existant ?** Sauvegardé dans `.agent-skills-backup/`, harness installé à neuf, votre `settings.json` restauré
- **Un répertoire `process/` existant ?** Jamais touché par l'installation — `ag-setup` gère la migration intelligemment
- **Un `CLAUDE.md` existant ?** Sauvegardé en `CLAUDE.md.pre-agent-skills`, version harness installée
...
L'agent ne fait pas que lire votre code — il **étudie d'autres dépôts** pour apprendre comment ils ont résolu des problèmes similaires (`ag-xia`).
...
Avant de router toute requête, analyse <strong>32 skills</strong> et fait correspondre les mots-clés. Dites "add webhook support" et <code>ag-security</code> + <code>ag-scenario</code> apparaissent automatiquement.
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
→ La découverte de skills révèle : ag-scenario, ag-security
...
→ Route vers ag-debugger → collecte de preuves → hypothèses concurrentes
...
Vous : "improve test coverage to 80% using ag-autoresearch"
...
<code>ag-predict</code> — Architecte, Sécurité, Performance, UX et Avocat du Diable débattent de votre plan. Produit un verdict <strong>GO / CAUTION / STOP</strong> avant que vous n'écriviez une seule ligne de code.
</td>
<td width="50%" valign="top">
<h1>🎲</h1>
<strong>Générateur de cas limites en 12 dimensions</strong><br><br>
<code>ag-scenario</code> — Décompose n'importe quelle fonctionnalité selon 12 dimensions (types d'utilisateurs, extrêmes d'entrée, timing, échelle, état, env, erreurs, auth, données, intégrations, conformité, logique métier). Les sorties sont utilisables comme specs de tests.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-security</code> — Audit de sécurité à double méthodologie avec audit de dépendances, détection de secrets, et <strong>mode auto-correctif</strong> qui trie par sévérité et corrige les Critiques en premier avec des gardes de régression.
</td>
</tr>
</table>
...
<code>ag-autoresearch</code> — Définissez un objectif, partez. Boucle itérative sauvegardée par git : faire UN changement atomique → commit → mesurer → garder ou annuler. La détection de blocage après 5 annulations consécutives déclenche des changements de stratégie.
</td>
<td width="50%" valign="top">
<h1>👥</h1>
<strong>Équipes d'agents parallèles</strong><br><br>
<code>ag-team</code> — Plusieurs agents travaillant <strong>simultanément</strong> avec isolation par git worktree. Recherche en parallèle, exécution en parallèle, revue en parallèle, débogage adversarial.
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<code>ag-debugger</code> — Collecte les preuves d'abord → forme 2-3 hypothèses concurrentes → teste chacune systématiquement → documente le chemin d'élimination. <strong>Ne devine jamais — prouve.</strong>
</td>
</tr>
</table>
...
**🔧 Skills contractuels** — `ag-generate-plan` · `ag-generate-context` · `ag-audit-context` · `ag-audit-plans` · `ag-audit-ag` · `ag-setup` · `ag-update` · `ag-publish`

**🧠 Planification** — `ag-predict` (débat à 5 personas) · `ag-scenario` (cas limites en 12 dimensions) · `ag-sequential-thinking` · `ag-problem-solving`

**🐛 Débogage & sécurité** — `ag-debug` · `ag-security` (STRIDE + OWASP + auto-correctif) · `ag-autoresearch` (optimisation autonome)

**📚 Recherche** — `ag-docs-seeker` · `ag-scout` · `ag-docs` · `ag-repomix` · `ag-xia` (comparaison de dépôts)

**🎨 Frontend** — `ag-frontend-design` · `ag-chrome-devtools` · `ag-agent-browser` · `ag-web-testing`

**⚙️ Utilitaires** — `ag-context-engineering` · `ag-mcp-management` · `ag-preview` · `ag-team` (agents parallèles) · `ag-tech-graph` · `ag-watzup` (handoff de session) · `ag-merge-worktree`

</details>

> 💡 Certains skills (comme `ag-xia`) ont été inspirés par [ClaudeKit](https://claudekit.cc/?ref=OEOM7R7G) de [@mrgoonie](https://github.com/mrgoonie). Nous nous sommes concentrés sur moins de skills, mais plus approfondis, plutôt que 80+.
...
Ce projet est cloné et personnalisé à partir de [vibecode-pro-max-kit](https://github.com/withkynam/vibecode-pro-max-kit) par [@withkynam](https://github.com/withkynam).
...
La différence : agent-skills-kit se concentre sur le framework de développement spec-driven et l'organisation de contexte auto-améliorante, sans vous surcharger de 80+ skills. Moins d'outils, plus de structure.
