...
# Đóng góp cho agent-skills-kit

Cảm ơn bạn đã quan tâm đến việc đóng góp cho agent-skills-kit! Project này cung cấp một agent harness sẵn dùng cho Claude Code và Codex, và chúng tôi hoan nghênh contributions từ mọi người.
...
- **Không** dùng prefix `ag-` -- prefix đó dành riêng cho official harness skills
...
File `ag-manifest.json` ở root repository theo dõi tất cả managed files trong harness. Khi bạn thêm files mới (skills, agents, hooks, protocols, seeds), bạn phải cập nhật manifest này.

Manifest được dùng bởi `install.sh` và `ag-update` để biết files nào cần copy và sync. Nếu file mới không có trong manifest, nó sẽ không được include khi users install hoặc update harness.
...
- [ ] Tên skill **không** dùng prefix `ag-` (dành riêng cho official skills)
- [ ] Helper scripts nằm trong thư mục con `scripts/`
- [ ] `ag-manifest.json` đã được cập nhật với tất cả file paths mới
- [ ] Validation pass:
...
Cảm ơn bạn đã giúp agent-skills-kit tốt hơn cho mọi người!
