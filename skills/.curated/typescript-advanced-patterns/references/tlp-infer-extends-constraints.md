...
By default, `infer X` captures whatever shape the position allows — usually `string`, `unknown`, or a wide union — and downstream rules then re-narrow with extra conditionals. TypeScript 4.7 added `infer X extends Y`, which both binds the variable _and_ asserts a constraint. The resulting type is exactly `Y`, not the wider position. This eliminates entire layers of post-narrowing and turns "parse-then-validate" type-level pipelines into single-step extractions.
...
- Companion to `[[tlp-template-literal-pattern-matching]]`: pattern matching identifies _where_ to extract, `infer extends` controls _what shape_ the extracted token must take.
