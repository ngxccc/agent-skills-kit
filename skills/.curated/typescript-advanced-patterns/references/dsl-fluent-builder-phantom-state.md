...
class QueryBuilder<S extends BuilderState = { table: false; columns: false }> {
  ...
  from(table: string): QueryBuilder<S & { table: true }> {
    return new QueryBuilder({ ...this.parts, table }) as QueryBuilder<
      S & { table: true }
    >;
  }

  select(columns: string[]): QueryBuilder<S & { columns: true }> {
    return new QueryBuilder({ ...this.parts, columns }) as QueryBuilder<
      S & { columns: true }
    >;
  }
...
- `typescript-refactor`'s `arch-branded-types` covers nominal typing for IDs. This rule applies the same nominal-tag mechanism to builder _state_, where the brand is a record of which methods have been called.
