...
**Stricter typing for CSS modules** — if you use a typegen step (`typed-css-modules`, `vite-plugin-css-modules-types`), it emits one `.d.ts` per CSS file with the _actual_ class names, and `styles.titlee` becomes a type error. The ambient declaration above is the fallback when typegen isn't wired up.
