...
The server schema change _immediately_ fails the client compile because the input type widened. No coordination, no drift.
...
2. **Parse on the way in _and_ on the way out.** Server parses the request body and the response. Client parses the request input and the response. Two-way parsing catches both directions of drift.
...
- Combines `[[dsl-schema-first-inference]]` (schema as source of types) with the _bilateral_ discipline that makes end-to-end typing actually catch drift on both sides.
