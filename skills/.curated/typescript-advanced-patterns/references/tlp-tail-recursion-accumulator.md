...
TypeScript's type checker bounds recursive conditional type instantiations at roughly 50 steps. Naïve `head:rest` recursion blows that limit on any non-trivial input. Since TypeScript 4.5, the checker detects tail-recursive conditional types and processes them iteratively, raising the limit to ~1000. The trick is to thread an _accumulator_ through the recursion so each step's result is part of the next call's input, never wrapped in another type constructor on return. Once a rule has this shape, it scales to long tuples, long strings, and deep paths.
...
type Length<T extends readonly unknown[]> = T extends readonly [
  ...
  ? Length<Rest> extends infer R extends number
...
- `typescript-refactor`'s `compile-avoid-deep-recursion` says "limit recursion depth." This rule explains _how_ to raise that limit when you actually need depth: the structural transformation must be rewritten in tail-recursive form, not just shortened.
