...
```typescript
function concat<T extends readonly unknown[][]>(...arrs: [...T]): ConcatAll<T> {
  return arrs.flat() as ConcatAll<T>;
}

type ConcatAll<T extends readonly unknown[][]> = T extends readonly [
...
The `[...infer A]` form preserves both _element types_ and _labels_ in tuples — labels survive when present.
...
- Variadic tuples are the structural primitive that makes the accumulator pattern in `[[tlp-tail-recursion-accumulator]]` ergonomic. This rule covers the _uses_ of variadics; the accumulator rule covers their _recursion behaviour_.
